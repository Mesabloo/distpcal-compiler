...
| Other | `Root modules` · `Driver/` · `Tests/` · `reference/` · `docs/` · `scripts/` · `.claude/` · `Root` |
...
## `scripts/`
Tooling run through `lake env lean --run`, claimed by no `lean_lib` — `lake build` never touches
it, so a broken script cannot break the build.
- `DumpFacts.lean` — extracts every hand-written local theorem, Prop-valued class field and
...
- `facts` — queries that database. `q` ranked search across all kinds, `c` by the constants a
  ...
  against the sources, `refresh` to re-extract and reindex. Rebuilds its SQLite index
  automatically when either source file is newer, so there is no stale-index step. Look here
  before searching the sources for a lemma; Mathlib is deliberately not indexed (that is what
  `Loogle` and `lean_local_search` are for).

## `.claude/`
- `plans/` — plan docs beyond `PLAN.md`.
- `tasklist.md` — running task list.
- `facts/` — local-fact database, queried through `scripts/facts`. `lemmas.jsonl` is generated by
  `scripts/DumpFacts.lean` and never hand-edited; `notes.jsonl` is curated through `facts add`
  and `facts rm`; `index.db` is derived from both.
- `settings.json`, `settings.local.json` — Claude Code settings. Neither is tracked in git.
- `hooks/` — Claude Code hook scripts, wired from `settings.json`.
  - `git-guardrails.sh` — `PreToolUse:Bash`. Hard-blocks mutating git (`commit`, `push`, `add`,
    `rm`, `reset`, `clean`, `checkout`, `stash`, rebase/merge family, plus destructive
    `branch`/`tag`/`worktree`/`remote` flags). Read-only git stays free. Not bypassable from the
    command line; only `FUGUE_GUARDRAILS_DISABLE=1` in the hook's own environment turns it off.
  - `facts-steer.sh` — `PreToolUse:Bash`. Steers `grep` for `theorem`/`lemma` to `scripts/facts`.
  - `lean-reminder.sh` — `PreToolUse:Read`.
  - `lean-style-check.sh` — `Stop`. Runs `scripts/lean-style`.
