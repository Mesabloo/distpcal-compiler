...
type* (not an arbitrary placeholder) — cheap to get right, and one less "this is fake" wrinkle
for anything that ever does inspect a builtin's body (`-d dump-typed`, e.g.). The one case
that can't be made truly type-correct is `Head`'s `a` (module doc on `naturalsDeclarations`/
`sequencesDeclarations` below): a rigid, universally-quantified type variable has no witness
...
binary `-` (`Elaborator/Declarations.lean`'s module doc on `PrefixOperator.canonicalName`). Every
body here is checked against the operator's own *return* type (`checkDeclaration`'s own rule for
`args ≠ []`), so `Int`-returning operators get `intZero`, `Bool`-returning comparisons get
`.true`, and `..`/`Nat` (both `Set(Int)`-returning) get `emptySetInt`. -/
...
/-- `Sequences`'s operators (module doc on `naturalsDeclarations`). `Len` returns `Int`
(`intZero`); `Tail`/`Append` return `Seq(a)`, genuinely well-typed for *any* `a` via the empty
sequence (`emptySeqOfVarA`) — no witness of `a` itself is needed to build one. `Head` returns
bare `a` — a rigid type variable with no witness value at all, so `intZero` here is the one
placeholder in this file that isn't actually well-typed (harmless: never re-checked, module
doc above). -/
