import PlusCalCompiler.Passes.GuardedToNetwork.PlusCal
...
namespace GuardedPlusCal
  protected structure AtomicBranch.wellFormed.{u}
      (inbox : String) (mailbox : Option (CoreTLAPlus.Expression CoreTLAPlus.Typ)) (procName : String)
      (fifos : Std.HashMap String (SurfaceTLAPlus.Typ × List (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)))
      (Br : AtomicBranch.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) : Prop where
  ...
  protected structure Process.wellFormed
      (P : Process CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) (inbox : String)
  ...
  protected structure Algorithm.wellFormed (A : Algorithm CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) («Σ» : Scope) (inbox : String) : Prop where
    «Σ_contains_prims» : CoreTLAPlus.prims.keys.toFinset ⊆ «Σ»
    ...
    inbox_fresh : ∀ P ∈ A.procs, match P.mailbox with
      | .none => True
      | .some _ => inbox ++ P.name ∉ «Σ» ∧ Process.FreshIn CoreTLAPlus.Expression.FreshIn P (inbox ++ P.name)
    A_wellscoped : A.WellScoped (flip CoreTLAPlus.Expression.WellScoped) «Σ»
  ...
  abbrev LocalState'.{u} := Memory.{u} × Memory.{u} × FIFOs.{u} × Option String
  abbrev LocalState'.mk.{u} (w x : Memory.{u}) (y : FIFOs.{u}) (z : Option String) : LocalState'.{u} := ⟨w, x, y, z⟩

  instance instReduceStatement' {b b' : Bool} : Reduce (Statement CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b') (Set (LocalState' × List Behavior × LocalState')) where
  ...
  instance instAbortStatement' {b b' : Bool} : Abort (Statement CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b') (Set (LocalState' × List Behavior)) where
    abort S := {⟨⟨M, T, F, l⟩, ε⟩ | l = .none ∧ ⟨.running M T F, ε⟩ ∈ ⟦S⟧⊥}
  instance instDivergeStatement' {b b' : Bool} : Diverge (Statement CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b') (Set (LocalState' × List Behavior)) where
  ...
  instance instReduceAtomicBranch' : Reduce (AtomicBranch SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState' × List Behavior × LocalState')) where
    reducing Br := {⟨⟨M, T, F, l⟩, ε, ⟨M', T', F', l'⟩⟩ | ∃ l'', ⟨.running M T F, ε, .done M' T' F' l''⟩ ∈ ⟦Br⟧* ∧ l = .none ∧ l' = .some l''}
  instance instAbortAtomicBranch' : Abort (AtomicBranch SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState' × List Behavior)) where
    abort Br := {⟨⟨M, T, F, l⟩, ε⟩ | l = .none ∧ ⟨.running M T F, ε⟩ ∈ ⟦Br⟧⊥}
  instance instDivergeAtomicBranch' : Diverge (AtomicBranch SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState' × List Behavior)) where
  ...
  instance : Reduce (AtomicBlock CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) (Set (LocalState' × List Behavior × LocalState')) where
    reducing B := {⟨st, ε, st'⟩ | ∃ Br ∈ B.branches, ⟨st, ε, st'⟩ ∈ ⟦Br⟧*}
  instance : Abort (GuardedPlusCal.AtomicBlock SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState' × List Behavior)) where
    abort B := {⟨st, ε⟩ | ∃ Br ∈ B.branches, ⟨st, ε⟩ ∈ ⟦Br⟧⊥}
  instance : Diverge (GuardedPlusCal.AtomicBlock SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState' × List Behavior)) where
    div B := {⟨st, ε⟩ | ∃ Br ∈ B.branches, ⟨st, ε⟩ ∈ ⟦Br⟧∞}

  -----------------

  private theorem mem_sem_iff.{u, v} (σₜ σₜ' : LocalState'.{v}) (ε : List GuardedPlusCal.Behavior)
    (S : Statement.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) false true)
...
namespace NetworkPlusCal
  ...
  instance instReduceAtomicBlock : Reduce (AtomicBlock CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) (Set (LocalState' × List Behavior × LocalState')) where
    reducing B := {⟨⟨M, T, F, l⟩, ε, ⟨M', T', F', l'⟩⟩ | ∃ Br ∈ B.branches, ∃ l'', ⟨.running M T F, ε, .done M' T' F' l''⟩ ∈ ⟦Br⟧* ∧ l = .none ∧ l' = .some l''}
  instance instAbortAtomicBlock : Abort (AtomicBlock CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) (Set (LocalState' × List Behavior)) where
    abort B := {⟨⟨M, T, F, l⟩, ε⟩ | ∃ Br ∈ B.branches, ⟨.running M T F, ε⟩ ∈ ⟦Br⟧⊥ ∧ l = .none}
  instance instDivergeAtomicBlock : Diverge (AtomicBlock CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)) (Set (LocalState' × List Behavior)) where
    div B := {⟨⟨M, T, F, l⟩, ε⟩ | ∃ Br ∈ B.branches, ⟨.running M T F, ε⟩ ∈ ⟦Br⟧∞ ∧ l = .none}

  @[reducible] instance instReduceStatement'.{u} {b b' : Bool} : Reduce (Statement.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b') (Set (LocalState'.{u} × List Behavior.{u} × LocalState'.{u})) where
    reducing S := {⟨⟨M, T, F, l⟩, ε, ⟨M', T', F', l'⟩⟩ | ∃ (σ' : LocalState b'), l = .none ∧ ⟨.running M T F, ε, σ'⟩ ∈ ⟦S⟧* ∧ match b' with | true => ∃ l'', σ' = .done M' T' F' l'' ∧ l' = .some l'' | false => σ' = .running M' T' F' ∧ l' = .none}
  @[reducible] instance instAbortStatement'.{u} {b b' : Bool} : Abort (Statement.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b') (Set (LocalState'.{u} × List Behavior.{u})) where
    abort S := {⟨⟨M, T, F, l⟩, ε⟩ | l = .none ∧ ⟨.running M T F, ε⟩ ∈ ⟦S⟧⊥}
  @[reducible] instance instDivergeStatement'.{u} {b b' : Bool} : Diverge (Statement.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b') (Set (LocalState'.{u} × List Behavior.{u})) where
    div S := {⟨⟨M, T, F, l⟩, ε⟩ | l = .none ∧ ⟨.running M T F, ε⟩ ∈ ⟦S⟧∞}

  instance instReduceAtomicBranch'.{u} : Reduce (AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState'.{u} × List Behavior.{u} × LocalState'.{u})) where
    reducing Br := {⟨⟨M, T, F, l⟩, ε, ⟨M', T', F', l'⟩⟩ | ∃ l'', ⟨.running M T F, ε, .done M' T' F' l''⟩ ∈ ⟦Br⟧* ∧ l = .none ∧ l' = .some l''}
  instance instAbortAtomicBranch'.{u} : Abort (AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState'.{u} × List Behavior.{u})) where
    abort Br := {⟨⟨M, T, F, l⟩, ε⟩ | l = .none ∧ ⟨.running M T F, ε⟩ ∈ ⟦Br⟧⊥}
  instance instDivergeAtomicBranch'.{u} : Diverge (AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)) (Set (LocalState'.{u} × List Behavior.{u})) where
    div Br := {⟨⟨M, T, F, l⟩, ε⟩ | l = .none ∧ ⟨.running M T F, ε⟩ ∈ ⟦Br⟧∞}
  ...
  private theorem mem_sem_iff₁.{u} (σₜ σₜ' : LocalState'.{u}) (ε : List GuardedPlusCal.Behavior)
    {b} (S : Statement.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b true) :
    (σₜ, ε, σₜ') ∈ ⟦S⟧* ↔ ∃ M T F M' T' F' l', σₜ = ⟨M, T, F, .none⟩ ∧ σₜ' = ⟨M', T', F', .some l'⟩ ∧ ⟨.running M T F, ε, .done M' T' F' l'⟩ ∈ instReduceStatement.reducing S where
      mp := by
        rintro ⟨_, σₜ_l_eq, _, l, rfl, σₜ'_l_eq⟩
        exists σₜ.fst, σₜ.snd.fst, σₜ.snd.snd.fst, σₜ'.fst, σₜ'.snd.fst, σₜ'.snd.snd.fst, l
  ...
  private theorem mem_sem_iff₂.{u} (σₜ σₜ' : LocalState'.{u}) (ε : List GuardedPlusCal.Behavior)
    {b} (S : Statement.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b false) :
...
theorem NetworkPlusCal.Block.sem_eq_map₃.{u}
  {b b' : Bool} {B : GuardedPlusCal.Block.{u} (NetworkPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b) b'} :
...
theorem GuardedPlusCal.Block.sem_eq_map₃.{u}
  {b b' : Bool} {B : GuardedPlusCal.Block.{u} (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b) b'} :
...
theorem NetworkPlusCal.LocalState.sem_glue₁.{u}
  {Mₜ₁ Mₜ₂ Tₜ₁ Tₜ₂ : CoreTLAPlus.Memory.{u}} {Fₜ₁ Fₜ₂ : NetworkPlusCal.FIFOs.{u}} {l : String} {ε₂ : List GuardedPlusCal.Behavior.{u}}
  {B : GuardedPlusCal.Block (NetworkPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) false) true} :
...
theorem NetworkPlusCal.LocalState.sem_glue₂.{u}
  {Mₜ₁ Mₜ₂ Tₜ₁ Tₜ₂ : CoreTLAPlus.Memory.{u}} {Fₜ₁ Fₜ₂ : NetworkPlusCal.FIFOs.{u}} {ε₂ : List GuardedPlusCal.Behavior.{u}}
  {B : GuardedPlusCal.Block (NetworkPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) true) false} :
...
theorem GuardedPlusCal.LocalState.sem_glue₁.{u}
  {b : Bool} {Mₜ₁ Mₜ₂ Tₜ₁ Tₜ₂ : CoreTLAPlus.Memory.{u}} {Fₜ₁ Fₜ₂ : NetworkPlusCal.FIFOs.{u}} {l : String} {ε₂ : List GuardedPlusCal.Behavior.{u}}
  {B : GuardedPlusCal.Block (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b) true} :
...
theorem GuardedPlusCal.LocalState.sem_glue₂.{u}
  {b : Bool} {Mₜ₁ Mₜ₂ Tₜ₁ Tₜ₂ : CoreTLAPlus.Memory.{u}} {Fₜ₁ Fₜ₂ : NetworkPlusCal.FIFOs.{u}} {ε₂ : List GuardedPlusCal.Behavior.{u}}
  {B : GuardedPlusCal.Block (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b) false} :
...
theorem GuardedPlusCal.LocalState.abort_glue.{u}
  {Mₛ₁ Tₛ₁ : CoreTLAPlus.Memory.{u}} {Fₛ₁ : GuardedPlusCal.FIFOs.{u}} {ε : List GuardedPlusCal.Behavior.{u}}
  {b b'} {B : GuardedPlusCal.Block (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b) b'} :
...
theorem NetworkPlusCal.LocalState.abort_glue.{u}
  {Mₛ₁ Tₛ₁ : CoreTLAPlus.Memory.{u}} {Fₛ₁ : NetworkPlusCal.FIFOs.{u}} {ε : List NetworkPlusCal.Behavior.{u}}
  {b b'} {B : GuardedPlusCal.Block (NetworkPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) b) b'} :
...
theorem GuardedPlusCal.AtomicBranch.sem_eq {Br : AtomicBranch SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
theorem GuardedPlusCal.AtomicBranch.abort_eq.{u} {Br : AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
theorem GuardedPlusCal.AtomicBranch.div_eq.{u} {Br : AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
    ⟦Br⟧∞ = (Br.precondition.elim ∅ (⟦·⟧∞)) ∪ (Br.precondition.elim {⟨x, ε, y⟩ | x = y ∧ ε = Trace.τ} (⟦·⟧*)) ∘ᵣ₁ ⟦Br.action⟧∞ := by
...
theorem NetworkPlusCal.AtomicBranch.sem_eq {Br : AtomicBranch SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
    ⟦Br⟧* = (Br.precondition.elim {⟨x, ε, y⟩ | x = y ∧ ε = Trace.τ} (⟦·⟧*)) ∘ᵣ₂ ⟦Br.action⟧* := by
  ext ⟨st, ε, st'⟩
  constructor
...
theorem NetworkPlusCal.AtomicBranch.abort_eq.{u} {Br : AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
    ⟦Br⟧⊥ = (Br.precondition.elim ∅ (⟦·⟧⊥)) ∪ (Br.precondition.elim {⟨x, ε, y⟩ | x = y ∧ ε = Trace.τ} (⟦·⟧*)) ∘ᵣ₁ ⟦Br.action⟧⊥ := by
...
theorem NetworkPlusCal.AtomicBranch.div_eq.{u} {Br : AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
    ⟦Br⟧∞ = (Br.precondition.elim ∅ (⟦·⟧∞)) ∪ (Br.precondition.elim {⟨x, ε, y⟩ | x = y ∧ ε = Trace.τ} (⟦·⟧*)) ∘ᵣ₁ ⟦Br.action⟧∞ := by
...
theorem NetworkPlusCal.LocalState.sem_glue₃.{u} {st : NetworkPlusCal.LocalState.{u} false} {st' : NetworkPlusCal.LocalState.{u} true} {ε} {Br : NetworkPlusCal.AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
theorem NetworkPlusCal.LocalState.abort_glue₂.{u} {st : NetworkPlusCal.LocalState.{u} false} {ε} {Br : NetworkPlusCal.AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
theorem NetworkPlusCal.AtomicBlock.div_eq.{u} {B : AtomicBlock.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
lemma NetworkPlusCal.LocalState'.sem_label_eq.{u} (B : Block.{u} (Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) true) false)
...
theorem NetworkPlusCal.LocalState.div_glue₃.{u} {st : NetworkPlusCal.LocalState.{u} false} {ε} {Br : NetworkPlusCal.AtomicBranch.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
theorem NetworkPlusCal.LocalState.div_glue₂.{u} {st : NetworkPlusCal.LocalState.{u} false} {ε} {Br : NetworkPlusCal.AtomicBlock.{u} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
...
namespace GuardedToNetwork
  ...
  def relatesTo.{u} (mbox : Option (CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ × String)) : Rel GuardedPlusCal.LocalState'.{u} NetworkPlusCal.LocalState'.{u} :=
    λ ⟨M₁, T₁, F₁, l₁⟩ ⟨M₂, T₂, F₂, l₂⟩ ↦
      ...
      | .some (.funcall (.var c) [e@(.var "self")], inbox) =>
        ...
        (∃ vs,
          ...
          M₂.lookup inbox = CoreTLAPlus.Value.seq vs ∧
          T₂.lookup inbox = none ∧
          ∃ v : CoreTLAPlus.Value.{u},
  ...
  private theorem GuardedPlusCal.Thread.toNetwork.proc_name_eq
    (Pₜ : NetworkPlusCal.Process SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))
    (Pₛ : GuardedPlusCal.Process SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))
    (chans : Std.HashMap String (SurfaceTLAPlus.Typ × List (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)))
  ...
  private theorem GuardedPlusCal.Thread.toNetwork.processPrecondition_spec₁ {nameₛ} {fifosₛ : Std.HashMap String (SurfaceTLAPlus.Typ × List (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))}
    {B : Option (GuardedPlusCal.Block _ _)} :
    ⦃⌜True⌝⦄
    GuardedPlusCal.Thread.toNetwork.processPrecondition inbox nameₛ fifosₛ B
    ⦃⇓ (_, _, _) => ⌜True⌝⦄ := id

  private theorem GuardedPlusCal.Thread.toNetwork_spec₁ {nameₛ} {fifosₛ : Std.HashMap String (SurfaceTLAPlus.Typ × List (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))}
    {Tₛ : GuardedPlusCal.Thread SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)} :
    ⦃⌜True⌝⦄
  ...
  abbrev ProcRefine.{u}
    (Pₛ : GuardedPlusCal.Process.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ))
    (Pₜ : NetworkPlusCal.Process.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ))
    : Prop :=
  ...
  theorem GuardedPlusCal.Thread.toNetwork.processPrecondition_spec₂
    {channelsₛ : Std.HashMap String (SurfaceTLAPlus.Typ × List (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))}
  ...
  theorem GuardedPlusCal.Thread.toNetwork.processBlock.last_eq
    {actionₛ : GuardedPlusCal.Block (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) false) true} :
  ...
  theorem GuardedPlusCal.Thread.toNetwork.processBlock.begin_eq_cons_if
    {actionₛ : GuardedPlusCal.Block (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) false) true} {S Ss}
  ...
  -- TODO: move into CoreTLAPlus
  theorem CoreTLAPlus.eval_ext {M₁ M₂ : CoreTLAPlus.Memory} {e : CoreTLAPlus.Expression CoreTLAPlus.Typ} (ys : List String)
    (mem_ext : ∀ x ∉ ys, M₁.lookup x = M₂.lookup x) (ys_not_in_freevars : List.Disjoint ys e.freeVars) :
    CoreTLAPlus.eval M₁ e = CoreTLAPlus.eval M₂ e := by
      trans CoreTLAPlus.eval (M₁.removeAll ys) e
      · clear mem_ext
        induction ys with
        | nil => rw [AList.removeAll_nil]
        | cons y ys IH =>
          rw [AList.removeAll_cons', IH]
          · apply CoreTLAPlus.eval_not_fv_irrel
            simp_all
          · simp_all
      · trans CoreTLAPlus.eval (M₂.removeAll ys) e
        · apply CoreTLAPlus.eval_mem_ext
          intro x
          by_cases x_in : x ∈ ys
  ...
  theorem CoreTLAPlus.not_mem_of_fresh {Typ} {e : CoreTLAPlus.Expression Typ} {x} (h : e.FreshIn x) :
      x ∉ e.freeVars := by
    fun_induction e.FreshIn x <;> (
      -- unfold CoreTLAPlus.Expression.FreshIn at h
  ...
  theorem CoreTLAPlus.fresh_of_wellscoped_of_not_mem {Typ} {e : CoreTLAPlus.Expression Typ} {x} {Γ : Scope} (h : e.WellScoped Γ) (h' : x ∉ Γ) : e.FreshIn x := by
    fun_induction CoreTLAPlus.Expression.WellScoped e Γ <;> (
      -- unfold CoreTLAPlus.Expression.WellScoped at h
      unfold CoreTLAPlus.Expression.FreshIn
    )
  ...
  theorem CoreTLAPlus.wellscoped_mono_of_subset {Typ} {e : CoreTLAPlus.Expression Typ} {Γ Γ' : Scope} (h : e.WellScoped Γ) (h' : Γ ⊆ Γ') : e.WellScoped Γ' := by
    fun_induction CoreTLAPlus.Expression.WellScoped e Γ <;> unfold CoreTLAPlus.Expression.WellScoped at ⊢
    case case1 =>
      apply_rules [h']
  ...
  lemma NetworkPlusCal.Statement.div_empty.{u} {b b'} {S : NetworkPlusCal.Statement.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b b'} :
      ⟦S⟧∞ = ∅ := by
    ext ⟨⟨M, T, F, l⟩, ε⟩
    iff_rintro ⟨rfl, _, _⟩ (_|_)

  lemma NetworkPlusCal.Block.div_empty.{u} {b b'} {B : GuardedPlusCal.Block.{u} (NetworkPlusCal.Statement CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) b) b'} :
      ⟦B⟧∞ = ∅ := by
    ext ⟨⟨M, T, F, l⟩, ε⟩
    iff_rintro h (_|_)
    · unfold_projs at h
  ...
  lemma NetworkPlusCal.AtomicBranch.div_empty.{u} {Br : NetworkPlusCal.AtomicBranch.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)} :
      ⟦Br⟧∞ = ∅ := by
    ext ⟨⟨M, T, F, l⟩, ε⟩
    iff_rintro h (_|_)
  ...
  lemma NetworkPlusCal.AtomicBlock.div_empty.{u} {Br : NetworkPlusCal.AtomicBlock.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)} :
      (⟦Br⟧∞ : Set.{u} _) = ∅ := by
    ext ⟨⟨M, T, F, l⟩, ε⟩
    iff_rintro h (_|_)
    · change ∃ _, _ at h
  ...
  lemma NetworkPlusCal.Statement.abort_skip.{u} :
      ⟦@NetworkPlusCal.Statement.skip.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧⊥ = ∅ := by
    ext ⟨⟨M, T, F, l⟩, ε⟩
    iff_rintro ⟨rfl, _, _⟩ (_|_)

  lemma GuardedPlusCal.Statement.strong_refinement.skip.{u} {mailbox : Option (CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ × String)} :
      StrongRefinement (· ∼[mailbox] ·)
        ⟦@GuardedPlusCal.Statement.skip.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧*
        ⟦@GuardedPlusCal.Statement.skip CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧⊥
        ⟦@GuardedPlusCal.Statement.skip.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧∞
        ⟦@NetworkPlusCal.Statement.skip.{u} CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧*
        ⟦@NetworkPlusCal.Statement.skip CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧⊥
        ⟦@NetworkPlusCal.Statement.skip CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ)⟧∞ := by
  ...
  lemma GuardedPlusCal.Statement.strong_refinement.print.{u}
    {mailbox : Option (CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ × String)}
    {e : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    (wf : match mailbox with | .none => True | .some ⟨_, inbox⟩ => e.FreshIn inbox) :
      StrongRefinement (· ∼[mailbox] ·)
        ⟦@GuardedPlusCal.Statement.print.{u} CoreTLAPlus.Typ _ e⟧*
        ⟦@GuardedPlusCal.Statement.print CoreTLAPlus.Typ _ e⟧⊥
        ⟦@GuardedPlusCal.Statement.print CoreTLAPlus.Typ _ e⟧∞
        ⟦@NetworkPlusCal.Statement.print.{u} CoreTLAPlus.Typ _ e⟧*
        ⟦@NetworkPlusCal.Statement.print CoreTLAPlus.Typ _ e⟧⊥
        ⟦@NetworkPlusCal.Statement.print CoreTLAPlus.Typ _ e⟧∞ := by
    ...
    · rintro ⟨Mₜ, Tₜ, Fₜ, lₜ⟩ ⟨Mₜ', Tₜ', Fₜ', lₜ'⟩ ε ⟨Mₛ, Tₛ, Fₛ, lₛ⟩ sim sem_Sₜ
      ...
      · match mailbox with
        ...
        | .some (_, inbox) =>
          rw [← eval_e_eq_v]
          apply CoreTLAPlus.eval_ext [inbox]
          · intros v v_not_in
          ...
          · rw [List.singleton_disjoint]
            apply CoreTLAPlus.not_mem_of_fresh
            exact wf
  ...
  lemma GuardedPlusCal.Statement.strong_refinement.assert.{u}
    ...
    (wf : match mailbox with | .none => True | .some ⟨_, inbox⟩ => e.FreshIn inbox) :
      StrongRefinement (· ∼[mailbox] ·)
        ⟦@GuardedPlusCal.Statement.assert.{u} CoreTLAPlus.Typ _ e⟧*
      ⟦@GuardedPlusCal.Statement.assert CoreTLAPlus.Typ _ e⟧⊥
      ⟦@GuardedPlusCal.Statement.assert CoreTLAPlus.Typ _ e⟧∞
        ⟦@NetworkPlusCal.Statement.assert.{u} CoreTLAPlus.Typ _ e⟧*
        ⟦@NetworkPlusCal.Statement.assert CoreTLAPlus.Typ _ e⟧⊥
        ⟦@NetworkPlusCal.Statement.assert CoreTLAPlus.Typ _ e⟧∞ := by
    rw [NetworkPlusCal.Statement.div_empty]
    apply StrongRefinement.ofNonDiverging
    · rintro ⟨Mₜ, Tₜ, Fₜ, lₜ⟩ ⟨Mₜ', Tₜ', Fₜ', lₜ'⟩ ε ⟨Mₛ, Tₛ, Fₛ, lₛ⟩ sim sem_Sₜ
  ...
  lemma GuardedPlusCal.Statement.strong_refinement.send.{u}
    {chan} {mailbox : Option (CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ × String)}
    {e : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    (wf : match mailbox with | .none => True | .some ⟨_, inbox⟩ => chan.name ≠ inbox ∧ (∀ arg ∈ chan.args, arg.FreshIn inbox) ∧ e.FreshIn inbox) :
      StrongRefinement (· ∼[mailbox] ·)
        ⟦@GuardedPlusCal.Statement.send.{u} CoreTLAPlus.Typ _ chan e⟧*
        ⟦@GuardedPlusCal.Statement.send CoreTLAPlus.Typ _ chan e⟧⊥
        ⟦@GuardedPlusCal.Statement.send CoreTLAPlus.Typ _ chan e⟧∞
        ⟦@NetworkPlusCal.Statement.send.{u} CoreTLAPlus.Typ _ chan e⟧*
        ⟦@NetworkPlusCal.Statement.send CoreTLAPlus.Typ _ chan e⟧⊥
        ⟦@NetworkPlusCal.Statement.send CoreTLAPlus.Typ _ chan e⟧∞ := by
    rw [NetworkPlusCal.Statement.div_empty]
    apply StrongRefinement.ofNonDiverging
    · rintro ⟨Mₜ, Tₜ, Fₜ, lₜ⟩ ⟨Mₜ', Tₜ', Fₜ', lₜ'⟩ ε ⟨Mₛ, Tₛ, Fₛ, lₛ⟩ sim sem_Sₜ
      left
  ...
  lemma GuardedPlusCal.Statement.strong_refinement.assign.{u}
    {ref} {mailbox : Option (CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ × String)}
    {e : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    ...
    (wellscoped : e.WellScoped («Σ» ∪ Γ ∪ Ξ) ∧ ref.name ∈ Γ ∧ ∀ arg ∈ ref.args, ∀ idx ∈ arg, idx.WellScoped («Σ» ∪ Γ ∪ Ξ)) :
      StrongRefinement (· ∼[mailbox] ·)
        ⟦@GuardedPlusCal.Statement.assign.{u} CoreTLAPlus.Typ _ ref e⟧*
        ⟦@GuardedPlusCal.Statement.assign CoreTLAPlus.Typ _ ref e⟧⊥
        ⟦@GuardedPlusCal.Statement.assign CoreTLAPlus.Typ _ ref e⟧∞
        ⟦@NetworkPlusCal.Statement.assign.{u} CoreTLAPlus.Typ _ ref e⟧*
        ⟦@NetworkPlusCal.Statement.assign CoreTLAPlus.Typ _ ref e⟧⊥
        ⟦@NetworkPlusCal.Statement.assign CoreTLAPlus.Typ _ ref e⟧∞ := by
    rw [NetworkPlusCal.Statement.div_empty]
    apply StrongRefinement.ofNonDiverging
    · rintro ⟨Mₜ, Tₜ, Fₜ, lₜ⟩ ⟨Mₜ', Tₜ', Fₜ', lₜ'⟩ ε ⟨Mₛ, Tₛ, Fₛ, lₛ⟩ sim sem_Sₜ
      left
        ...
        · obtain ⟨rfl, mem_ext, rfl, vs, inbox_eq, inbox_not_in_Tₛ, es, eval_self, _, _, _⟩ := sim
          ...
          · and_intros <;> try rfl
            ...
            · exists vs
              ...
              · rwa [AList.lookup_insert_ne (Ne.symm ref_name_ne)]
              · refine ⟨es, ?_, ?_, ?_, ?_⟩
                · rw [← eval_self, CoreTLAPlus.eval_ext [ref.name]]
                  · intros x x_neq
  ...
  theorem GuardedPlusCal.Statement.strong_refinement (S : GuardedPlusCal.Statement CoreTLAPlus.Typ (CoreTLAPlus.Expression CoreTLAPlus.Typ) false false)
    {mailbox : Option (CoreTLAPlus.Expression SurfaceTLAPlus.Typ × String)}
    {«Σ» Γ Δ Ξ : Scope}
    (self_in_Ξ : "self" ∈ Ξ) (Γ_disj_Ξ : Disjoint Γ Ξ)
    (wellscoped : match S with
      | .skip => True
      | .print e => CoreTLAPlus.Expression.WellScoped e («Σ» ∪ Γ ∪ Ξ)
      | .assert e => CoreTLAPlus.Expression.WellScoped e («Σ» ∪ Γ ∪ Ξ)
      | .send chan e =>
        CoreTLAPlus.Expression.WellScoped e («Σ» ∪ Γ ∪ Ξ) ∧ chan.name ∈ Δ ∧ (∀ idx ∈ chan.args, CoreTLAPlus.Expression.WellScoped idx («Σ» ∪ Γ ∪ Ξ))
      | .multicast chan bs e =>
        chan ∈ Δ ∧ (∀ r ∈ bs, CoreTLAPlus.Expression.WellScoped r.2.2.2 («Σ» ∪ Γ ∪ Ξ)) ∧ (∀ r ∈ bs, r.1 ∉ «Σ» ∧ r.1 ∉ Γ ∧ r.1 ∉ Ξ) ∧
        CoreTLAPlus.Expression.WellScoped e («Σ» ∪ Γ ∪ Ξ ∪ (bs.map Prod.fst).toFinset)
      | .assign ref e =>
        CoreTLAPlus.Expression.WellScoped e («Σ» ∪ Γ ∪ Ξ) ∧ ref.name ∈ Γ ∧ (∀ arg ∈ ref.args, ∀ idx ∈ arg, CoreTLAPlus.Expression.WellScoped idx («Σ» ∪ Γ ∪ Ξ)))
    (wf : match mailbox with | .none => True | .some ⟨_, inbox⟩ => S.FreshIn CoreTLAPlus.Expression.FreshIn inbox) :
  ...
  theorem CoreTLAPlus.doUpdate_eq_doUpdate {v₂ v' : CoreTLAPlus.Value} {vss : List (List CoreTLAPlus.Value)} :
      GuardedPlusCal.Memory.updateRef.doUpdate v₂ v' vss = CoreTLAPlus.eval.doUpdate v' (Sum.inl <$> vss) v₂ := by
  ...
  theorem NetworkPlusCal.Statement.reorder_let {r} {e e': CoreTLAPlus.Expression SurfaceTLAPlus.Typ} {name} {τ : CoreTLAPlus.Typ} {«=|∈»}
    (h₁ : e'.FreshIn name) (h₂ : r.name ∉ CoreTLAPlus.prims) (h₃ : ∀ arg ∈ r.args, ∀ idx ∈ arg, idx.FreshIn name) (h₄ : r.name ≠ name) :
      ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧* ∘ᵣ₂ ⟦NetworkPlusCal.Statement.let name τ «=|∈» e⟧* =
      ⟦NetworkPlusCal.Statement.let name τ «=|∈» ((↿e.replace) <| r.substOf e')⟧* ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧* := by
    ...
    · rintro ⟨⟨Mb, Tb, Fb, _⟩, ε₁, ε₂, ⟨⟨_, _, _⟩, rfl, red_assign, _|_, rfl⟩, ⟨⟨_, _, _⟩, -, red_let, _|_, rfl⟩, rfl⟩
      ...
      · obtain ⟨v', v'_in, _, _, _|_⟩ := h
        ...
        · simp_rw [GuardedPlusCal.Memory.updateRef, Option.bind_eq_bind, Option.bind_eq_some_iff, Option.pure_def] at upd_Ma_eq_Mc
          ...
          change CoreTLAPlus.eval _ (CoreTLAPlus.Expression.replace e _ _) = _
          ...
          · unfold GuardedPlusCal.Ref.substOf
            ...
            · rw [List.isEmpty_iff] at args_empty
              ...
              rw [CoreTLAPlus.eval_mem_ext this]
            · apply CoreTLAPlus.eval_mem_ext
              intros x
          ...
          · unfold GuardedPlusCal.Ref.substOf
            ...
            · -- TODO: extract_goal?
              ...
              have : ?x = .some [(Sum.inl <$> vss, v₂)] := by
                ...
                conv_lhs =>
                  apply List.traverse_attach_eq_traverse' λ x ↦ Option.map Sum.inl (List.traverse (CoreTLAPlus.eval (Ta ∪ Ma)) x)
                ...
                | @cons es vs _ _ eval_es_vs _ IH =>
                  ...
                  have : List.traverse (CoreTLAPlus.eval (Ta ∪ Ma)) es = .some vs := by
              ...
              change List.foldlM (λ fn x ↦ CoreTLAPlus.eval.doUpdate fn x.1 x.2) v' [(Sum.inl <$> vss, v₂)] =
                     GuardedPlusCal.Memory.updateRef.doUpdate v₂ v' vss
              simp_rw [List.foldlM_cons, List.foldlM_nil, bind_pure]
              symm
              apply CoreTLAPlus.doUpdate_eq_doUpdate
        ...
        · exact vss
        · rw [← eval_e']
          apply CoreTLAPlus.eval_ext [name]
          · intros x x_not_in
            rw [List.not_mem_singleton] at x_not_in
    ...
    · rintro ⟨⟨Mb, Tb, Fb, _⟩, ε₁, ε₂, ⟨⟨_, _, _⟩, rfl, red_let, _|_, rfl⟩, ⟨⟨_, _, _⟩, -, red_assign, _|_, rfl⟩, rfl⟩
      ...
      · obtain ⟨v', v'_in, _, _, _|_⟩ := h
        ...
        · simp_rw [GuardedPlusCal.Memory.updateRef, Option.bind_eq_bind, Option.bind_eq_some_iff, Option.pure_def] at upd_Ma_eq_Mc
          obtain ⟨v', lookup_Mₜ, v'', upd_Ma_eq_Mc, _|_⟩ := upd_Ma_eq_Mc
          change CoreTLAPlus.eval _ (CoreTLAPlus.Expression.replace e _ _) = _ at eval_e
          rw [← eval_e, CoreTLAPlus.eval_subst (v := v'')]
          · unfold GuardedPlusCal.Ref.substOf
          ...
          · unfold GuardedPlusCal.Ref.substOf
            ...
            · rw [List.isEmpty_iff] at args_empty
              ...
              · rw [List.singleton_disjoint]
                ...
                assumption
            · -- TODO: extract goal?
              replace eval_e' : CoreTLAPlus.eval (Ta ∪ Ma) e' = some v₂ := by
                rw [← eval_e']
                apply CoreTLAPlus.eval_ext [name]
                · intros x x_not_in
                  rw [List.not_mem_singleton] at x_not_in
              ...
              replace eval_args_vss : List.Forall₂ (List.Forall₂ λ e v ↦ CoreTLAPlus.eval (Ta ∪ Ma) e = some v) r.args vss := by
                rw [← List.forall₂_iff_forall₂_attach] at eval_args_vss ⊢
                apply eval_args_vss.imp
                rintro ⟨arg, arg_in⟩ ⟨vs, vs_in⟩ eval_arg_vs
                rw [← List.forall₂_iff_forall₂_attach] at eval_arg_vs ⊢
  ...
  theorem NetworkPlusCal.Statement.reorder_await {r} {e e': CoreTLAPlus.Expression.{0} SurfaceTLAPlus.Typ} (h' : r.name ∉ CoreTLAPlus.prims.{0}) :
      ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧* ∘ᵣ₂ ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e⟧* =
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) ((↿e.replace) <| r.substOf e')⟧* ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧* := by
    ext ⟨⟨Ma, Ta, Fa, _⟩, ε, ⟨Mc, Tc, Fc, _⟩⟩
    constructor
  ...
  theorem GuardedPlusCal.Memory.updateRef.doUpdate_empty_args.{u} {v v' : CoreTLAPlus.Value.{u}} : GuardedPlusCal.Memory.updateRef.doUpdate v v' [] = pure v := by
    unfold GuardedPlusCal.Memory.updateRef.doUpdate
    simp

  theorem CoreTLAPlus.eval.doUpdate_empty_args.{u} {v v' : CoreTLAPlus.Value.{u}} :
      CoreTLAPlus.eval.doUpdate v [] v' = pure v' := by
  ...
  theorem GuardedPlusCal.Memory.updateRef.doUpdate_cons.{u} {v v' v'' : CoreTLAPlus.Value.{u}} {vs vss}
    (h : GuardedPlusCal.Memory.updateRef.doUpdate v v'' (vs :: vss) = some v') :
      let tup := match (motive := _ → CoreTLAPlus.Value.{u}) vs with | [v] => v | _ => CoreTLAPlus.Value.tuple vs
  ...
  theorem NetworkPlusCal.Statement.reorder_await'.aux_eval_except.{u}
    ...
    (eval_args : List.traverse (List.traverse (CoreTLAPlus.eval (T ∪ M))) r.args = some vss)
    ...
    | @cons arg vs args vss eval_arg eval_args IH =>
      ...
      · obtain ⟨v''', upd_eq, rfl⟩ := upd_eq
        ...
        rw [List.traverse_attach_eq_traverse' (f := λ x ↦ Sum.inl <$> List.traverse (CoreTLAPlus.eval (T ∪ M)) x),
  ...
  theorem NetworkPlusCal.Statement.reorder_await'.{u}
    ...
    (h' : r.name ∉ CoreTLAPlus.prims.{u}) :
      ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧⊥ ∪
        ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧* ∘ᵣ₁ ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e⟧⊥ ⊇
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) ((↿e.replace) <| r.substOf e')⟧⊥ ∪
        ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) ((↿e.replace) <| r.substOf e')⟧* ∘ᵣ₁ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e'⟧⊥ := by
    have generic_reorder_await' {e'' : CoreTLAPlus.Expression.{0} SurfaceTLAPlus.Typ}
      ...
      (eval_imp_eval : ∀ M T {v v' vss}, List.traverse (List.traverse (CoreTLAPlus.eval (T ∪ M))) r.args = some vss
          ...
          → T ∪ M ⊢ e'' ⇒ v') :
        ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e'⟧⊥
        ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e'⟧*
          ∘ᵣ₁ ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) e⟧⊥ ⊇
        ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) (e.replace r.name e'')⟧⊥
        ∪ ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) (e.replace r.name e'')⟧*
          ∘ᵣ₁ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e'⟧⊥ := by as_aux_lemma =>
          ...
          · by_cases eval_e' : ∃ v, Ta ∪ Ma ⊢ e' ⇒ v
            · obtain ⟨v, eval_e'⟩ := eval_e'
              ...
              · obtain ⟨r_name_in_Ma, r_name_not_in_Ta⟩ := r_name_in_mem
                ...
                by_cases eval_args : ∃ vss, List.traverse (List.traverse (CoreTLAPlus.eval (Ta ∪ Ma))) r.args = some vss
                · obtain ⟨vss, eval_args⟩ := eval_args
                  ...
                  · obtain ⟨v', upd_eq⟩ := upd
                    ...
                    · exists rfl
                      ...
                      rw [← neval_e_subst, CoreTLAPlus.eval_subst h']
                      · apply CoreTLAPlus.eval_mem_ext
                        intro x
            ...
            · push_neg at eval_e'
              ...
              left; left; right
              exists Ma, Ta, Fa
          · rw [CoreTLAPlus.eval_subst_of_not_mem_fv h' r_name_in_fv_e] at neval_e_subst
            by_cases eval_e' : ∃ v, Ta ∪ Ma ⊢ e' ⇒ v
            · obtain ⟨v, eval_e'⟩ := eval_e'
              by_cases r_name_in_mem : r.name ∈ Ma ∧ r.name ∉ Ta
              · obtain ⟨r_name_in_Ma, r_name_not_in_Ta⟩ := r_name_in_mem
                ...
                · obtain ⟨vss, eval_args⟩ := eval_args
                  ...
                  · obtain ⟨v', upd_eq⟩ := upd
                    ...
                    · exists rfl
                      ...
                      exists Mb, Ta, Fa, ?_, ?_ <;> try rfl
                      rw [← neval_e_subst]
                      apply CoreTLAPlus.eval_ext [r.name]
                      · intros x x_neq
                        rw [List.not_mem_singleton] at x_neq
              ...
              · obtain ⟨vss, eval_args⟩ := eval_args
                ...
                · obtain ⟨v'', upd_eq'⟩ := upd
                  ...
                  · exists rfl
                    ...
                    rw [← eval_e_subst, CoreTLAPlus.eval_subst h']
                    · apply CoreTLAPlus.eval_mem_ext
                      intros x
                      rw [AList.union_insert_ext_of_not_mem r_name_not_in_Ta]
      ...
      · obtain ⟨⟨_, _, _⟩, rfl, ⟨_, _, _, _|_, _|_, eval_e, rfl⟩, _|_, rfl⟩ := red_await
        ...
        · by_cases r_name_in_fv_e : r.name ∈ e.freeVars
          · obtain ⟨_, _, _, neval_e', _|_, rfl⟩ := abort_assign
            rw [CoreTLAPlus.neval_subst_of_mem_fv_of_neval r_name_in_fv_e] at eval_e
            · contradiction
  ...
  theorem NetworkPlusCal.Statement.sem_await_congr.{u} {e₁ e₂ : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ} (h : ∀ M : GuardedPlusCal.Memory.{u}, CoreTLAPlus.eval M e₁ = CoreTLAPlus.eval M e₂) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₁⟧* = ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₂⟧* := by
  ...
  theorem NetworkPlusCal.Statement.sem_await_congr'.{u} {e₁ e₂ : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ} (h : ∀ M : GuardedPlusCal.Memory.{u}, M ⊢ e₁ ⇒ .bool true ↔ M ⊢ e₂ ⇒ .bool true) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₁⟧* = ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₂⟧* := by
    ext ⟨⟨Ma, Ta, Fa, _⟩, _, ⟨Mb, Tb, Fb, _⟩⟩
  ...
  theorem NetworkPlusCal.Statement.sem_await_of_imp.{u} {e₁ e₂ : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ} (h : ∀ M : GuardedPlusCal.Memory.{u}, M ⊢ e₁ ⇒ .bool true → M ⊢ e₂ ⇒ .bool true) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₁⟧* ⊆ ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₂⟧* := by
  ...
  theorem NetworkPlusCal.Statement.abort_await_congr.{u} {e₁ e₂ : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    (h₂ : ∀ M : GuardedPlusCal.Memory.{u}, CoreTLAPlus.eval M e₁ = CoreTLAPlus.eval M e₂) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₁⟧⊥ = ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₂⟧⊥ := by
    ext ⟨⟨Ma, Ta, Fa, _⟩, _⟩
    constructor
  ...
  theorem NetworkPlusCal.Statement.abort_await_of_imp.{u} {e₁ e₂ : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    (h₂ : ∀ (M : GuardedPlusCal.Memory.{u}) v, (∀ b, v ≠ .some (.bool b)) → CoreTLAPlus.eval M e₁ = v → CoreTLAPlus.eval M e₂ = v) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₁⟧⊥ ⊆ ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e₂⟧⊥ := by
  ...
  theorem NetworkPlusCal.Statement.reorder_let'.{u} {name «=|∈»} {τ : SurfaceTLAPlus.Typ} {r} {e e': CoreTLAPlus.Expression.{0} SurfaceTLAPlus.Typ}
  ...
  theorem AtomicBranch.correctness_precond_let.{u} {nameₛ} {mailboxₛ} {name} {τ : CoreTLAPlus.Typ.{u}} {«=|∈»} {e : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    (wf : match mailboxₛ with | none => True | some _ => inbox ++ nameₛ ≠ name ∧ CoreTLAPlus.Expression.FreshIn e (inbox ++ nameₛ))
    --(wf : match mailboxₛ with | none => True | some _ => inbox ++ nameₛ ∉ (GuardedPlusCal.Statement.let pos name τ «=|∈» e).freeVars CoreTLAPlus.Expression.freeVars)
    (wf' : name ≠ "self") :
          ...
          | cases h
            ...
            · first
                | obtain ⟨vs, _, _, v, eval_self, _, _, _⟩ := sim_σₛ_σₜ.right.right
                  ...
                  · rw [← eval_self, CoreTLAPlus.eval_var, CoreTLAPlus.eval_var]
    ...
    1,4:
      ...
      | some mailboxₛ =>
        ...
        split at sim_σₛ_σₜ using _ | h | h <;> solve
          | contradiction
          | cases h
            apply CoreTLAPlus.eval_ext [inbox ++ nameₛ]
  ...
  theorem AtomicBranch.correctness_precond_await.{u} {nameₛ} {mailboxₛ} {e : CoreTLAPlus.Expression.{u} CoreTLAPlus.Typ}
    (wf : match mailboxₛ with | none => True | some _ => CoreTLAPlus.Expression.FreshIn e (inbox ++ nameₛ)) : -- (GuardedPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) pos e).freeVars CoreTLAPlus.Expression.freeVars) :
      StrongRefinement.Terminating (· ∼[Option.map (λ x ↦ (x, inbox ++ nameₛ)) mailboxₛ] ·)
        ⟦GuardedPlusCal.Statement.await.{u} (Typ := CoreTLAPlus.Typ) e⟧* ⟦GuardedPlusCal.Statement.await.{u} (Typ := CoreTLAPlus.Typ) e⟧⊥
        ⟦NetworkPlusCal.Statement.await.{u} (Typ := CoreTLAPlus.Typ) e⟧* := by
    ...
    · cases mailboxₛ with
      ...
      | some mailboxₛ =>
        trans CoreTLAPlus.eval (Tₜ ∪ Mₜ) e
        · apply CoreTLAPlus.eval_ext [inbox ++ nameₛ]
  ...
  theorem AtomicBranch.correctness_precond_receive {nameₛ : String} {mailboxₛ : CoreTLAPlus.Expression.{0} SurfaceTLAPlus.Typ}
    ...
    (wf : GuardedPlusCal.Statement.FreshIn.{0} CoreTLAPlus.Expression.FreshIn (GuardedPlusCal.Statement.receive (Typ := SurfaceTLAPlus.Typ) chan ref) (inbox ++ nameₛ))
    (wf₂ : ref.name ≠ "self")
    (wf' : match mailboxₛ with
      ...
      StrongRefinement.Terminating (· ∼[.some (mailboxₛ, inbox ++ nameₛ)] ·)
        ⟦GuardedPlusCal.Statement.receive (Typ := SurfaceTLAPlus.Typ) chan ref⟧*
        ⟦GuardedPlusCal.Statement.receive (Typ := SurfaceTLAPlus.Typ) chan ref⟧⊥
        (⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := SurfaceTLAPlus.Typ) (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«>» (.nat "0"))⟧* ∘ᵣ₂
         ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) ref (CoreTLAPlus.Expression.opcall (.var "Head") [.var (inbox ++ nameₛ)])⟧* ∘ᵣ₂
         ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) ⟨inbox ++ nameₛ, []⟩ (CoreTLAPlus.Expression.opcall (Typ := SurfaceTLAPlus.Typ) (.var "Tail") [.var (inbox ++ nameₛ)])⟧*) := by
    ...
    obtain ⟨_, _, eval_inbox_eq, vs, rfl⟩ := CoreTLAPlus.eval_Head_to_is_nonempty_seq CoreTLAPlus.eval_Head eval_Head_inbox
    obtain ⟨_, v', vs', eval_inbox_eq', rfl⟩ := CoreTLAPlus.eval_Tail_is_seq CoreTLAPlus.eval_Tail eval_Tail_inbox
    ...
    have : CoreTLAPlus.eval (Taₜ ∪ Mbₜ) (.var (inbox ++ nameₛ)) = CoreTLAPlus.eval (Taₜ ∪ Maₜ) (.var (inbox ++ nameₛ)) := by
      ...
      · -- TODO: extract as lemma?
        ...
        simp_rw [AList.union_erase, AList.erase_insert, CoreTLAPlus.eval_var]
        split_ifs <;> try trivial
        simp_rw [AList.lookup_union, AList.lookup_erase_ne (Ne.symm ref_name_neq)]
    erw [this] at eval_inbox_eq'
    obtain _|_ := CoreTLAPlus.eval_det eval_inbox_eq eval_inbox_eq'
    clear this eval_inbox_eq'
    unfold CoreTLAPlus.eval at eval_inbox_eq
    dsimp [CoreTLAPlus.primFromName] at eval_inbox_eq
    ...
    · obtain ⟨mem_ext, rfl, vs, lookup_inbox, inbox_not_in_Tₛ, v₀, eval_es_vs', fifos_ext₁, fifos_ext₂, lookup_mailbox⟩ := sim_σₛ_σₜ
      ...
      | none =>
        ...
        · rwa [List.forall₂_singleton, CoreTLAPlus.eval_var_pos_irrel]
        · assumption
      | some vs' =>
          ...
          · refine ⟨rfl, ?_, rfl, vs, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
            ...
            · rw [← eval_es_vs', CoreTLAPlus.eval_ext [ref.name]]
              · intros x x_neq
                ...
                rw [AList.lookup_union, AList.lookup_union, AList.lookup_insert_ne x_neq]
              · rwa [List.singleton_disjoint, CoreTLAPlus.Expression.freeVars, List.not_mem_singleton]
            · intros c' c'_neq es'
  ...
  lemma GuardedPlusCal.Statement.strong_refinement.await.{u} {mailbox} {e : CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ}
    (wf : match mailbox with | .none => True | .some ⟨_, inbox⟩ => e.FreshIn inbox) :
      StrongRefinement.Aborting (· ∼[mailbox] ·)
        ⟦GuardedPlusCal.Statement.await.{u} (Typ := SurfaceTLAPlus.Typ) e⟧⊥
        ⟦NetworkPlusCal.Statement.await.{u} (Typ := SurfaceTLAPlus.Typ) e⟧⊥ := by
    rintro ⟨Mₜ, Tₜ, Fₜ, lₜ⟩ ε ⟨Mₛ, Tₛ, Fₛ, lₛ⟩ sim ⟨rfl, ⟨_, _, _, eval_e_eq_v, _|_, rfl⟩|⟨_, _, _, v, v_not_bool, eval_e_eq_v, _|_, rfl⟩⟩
      <;> exists [], le_rfl, relatesTo_eq_label sim
      <;> [ (left; use Mₛ, Tₛ, Fₛ, ?_, rfl)
          | (right; use Mₛ, Tₛ, Fₛ, v, v_not_bool, ?_, rfl) ]
  ...
  theorem AtomicBranch.correctness_precond_await'
    ...
    {e : CoreTLAPlus.Expression.{0} SurfaceTLAPlus.Typ}
    {«Σ» Γ Δ Ξ : Scope}
    ...
    (e_wellscoped : CoreTLAPlus.Expression.WellScoped e («Σ» \ {"self"} ∪ Γ \ {"self"} ∪ Ξ))
    ...
    (inv : match (motive := _ → Prop) B with
      ...
      | .some B => StrongRefinement (· ∼[Option.map (fun x ↦ (x, inbox ++ nameₛ)) mailboxₛ] ·)
        ...
        (List.foldr (λ S sem ↦ ⟦S⟧⊥ ∪ ⟦S⟧* ∘ᵣ₁ sem) ∅ Ss)
        (List.foldr (λ S sem ↦ ⟦S⟧∞ ∪ ⟦S⟧* ∘ᵣ₁ sem) ∅ Ss)
        (⟦B⟧* ∘ᵣ₂ List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} newInstrs)
        (⟦B⟧⊥ ∪ ⟦B⟧* ∘ᵣ₁ newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) ∅)
        (⟦B⟧∞ ∪ ⟦B⟧* ∘ᵣ₁ newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧∞ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) ∅)) :
    ----
      ...
      StrongRefinement (· ∼[Option.map (λ x ↦ (x, inbox ++ nameₛ)) mailboxₛ] ·)
        ...
        (B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₂
          ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧* ∘ᵣ₂
          List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} newInstrs)
        (B.elim ∅ (⟦·⟧⊥) ∪
          B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₁
            ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧⊥ ∪
          (B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₂
            ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧*) ∘ᵣ₁
            newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) ∅)
        (B.elim ∅ (⟦·⟧∞) ∪
          B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₁
            ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧∞ ∪
          (B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₂
            ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧*) ∘ᵣ₁
            newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧∞ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) ∅) := by {
    ...
    have reorder_await_newInstrs :
        ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace (GuardedPlusCal.Ref.substOf x.1 x.2).1 (GuardedPlusCal.Ref.substOf x.1 x.2).2) e newInstrs)⟧* ∘ᵣ₂
          List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {⟨x, e, y⟩ | x = y ∧ e = Trace.τ} newInstrs =
        List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {⟨x, e, y⟩ | x = y ∧ e = Trace.τ} newInstrs ∘ᵣ₂
          ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) e⟧* := by
      ...
      | cons newInstr newInstrs IH =>
        ...
        have : List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) ({x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) newInstr.1 newInstr.2⟧*) newInstrs =
            ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) newInstr.1 newInstr.2⟧* ∘ᵣ₂ List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} newInstrs := by
          rw [← List.foldl_map]
    ...
    · have B_div_empty : B.elim (∅ : Set (_ × _)) (⟦·⟧∞) = ∅ := by
      ...
      have newInstrs_div_empty : newInstrs.foldr (init := ∅)
          (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧∞ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) = ∅ := by
        clear * -
        induction newInstrs with
        | nil =>
          rfl
      ...
      · rw [List.foldr_concat, Relation.lcomp₁.right_empty_eq_empty, Set.union_empty, abort_assoc']
        ...
        -- reorder await
        have reorder_await_newInstrs' :
            ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧⊥ ∪
              ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧* ∘ᵣ₁
                newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) ∅ ⊆
            newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem)
              ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) e⟧⊥ := by
        ...
        · conv =>
          ...
          · apply GuardedPlusCal.Statement.strong_refinement.await
            ...
            have wf' : match (motive := _ → Prop) mailboxₛ with
                | none => True
                | some _ => GuardedPlusCal.Statement.FreshIn CoreTLAPlus.Expression.FreshIn (GuardedPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) e) (inbox ++ nameₛ) := by
              cases mailboxₛ with
              | none => apply True.intro
              | some val =>
                apply wf
  ...
  lemma GuardedPlusCal.Statement.strong_refinement.let.{u} {mailbox} {name «=|∈»} {τ : SurfaceTLAPlus.Typ} {e : CoreTLAPlus.Expression.{u} SurfaceTLAPlus.Typ}
  ...
  theorem AtomicBranch.correctness_precond_let'
    ...
    {precondₛ : GuardedPlusCal.Block.{0} (GuardedPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) true) false}
    {Ss Ss' : List (GuardedPlusCal.Statement.{0} SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) true false)}
    {name : String} {τ : SurfaceTLAPlus.Typ} {«=|∈» : Bool} {e : CoreTLAPlus.Expression.{0} SurfaceTLAPlus.Typ}
    {«Σ» Γ Δ Ξ : Scope}
    {«Σ_disj_Δ» : Disjoint («Σ» \ {"self"}) (Δ \ {"self"})} {«Σ_disj_Γ» : Disjoint («Σ» \ {"self"}) (Γ \ {"self"})} {Δ_disj_Γ : Disjoint (Δ \ {"self"}) (Γ \ {"self"})}
    («prims_in_Σ» : CoreTLAPlus.prims.keys.toFinset ⊆ «Σ» \ {"self"})
    (e_wellscoped : CoreTLAPlus.Expression.WellScoped e («Σ» \ {"self"} ∪ Γ \ {"self"} ∪ Ξ))
    ...
    have reorder_let :
        ⟦NetworkPlusCal.Statement.let name τ «=|∈» (List.foldr (λ x e ↦ e.replace x.1 x.2) e (List.map (λ x ↦ GuardedPlusCal.Ref.substOf x.1 x.2) newInstrs))⟧* ∘ᵣ₂
          List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} newInstrs =
        List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} newInstrs ∘ᵣ₂
          ⟦NetworkPlusCal.Statement.let name τ «=|∈» e⟧* := by
      rw [List.foldr_map]
    ...
    · intros k k_lt
      ...
      and_intros
      · intros arg arg_in idx idx_in
        apply CoreTLAPlus.wellscoped_mono_of_subset (args_wellscoped _ arg_in _ idx_in)
        apply Finset.union_subset_union_right
        exact Finset.subset_insert _ _
      · apply CoreTLAPlus.wellscoped_mono_of_subset e_wellscoped
        apply Finset.union_subset_union_right
        exact Finset.subset_insert _ _
      · assumption
      · assumption
      ...
      · rw [List.foldr_concat, Relation.lcomp₁.right_empty_eq_empty, Set.union_empty, abort_assoc']
        ...
        · conv =>
          ...
          · apply GuardedPlusCal.Statement.strong_refinement.let
            ...
            have wf' : match (motive := _ → Prop) mailboxₛ with
                | none => True
                | some _ => GuardedPlusCal.Statement.FreshIn CoreTLAPlus.Expression.FreshIn (GuardedPlusCal.Statement.let name τ «=|∈» e) (inbox ++ nameₛ) := by
              cases mailboxₛ with
              | none => apply True.intro
              | some val =>
                apply wf
  ...
  theorem GuardedPlusCal.Statement.strong_refinement.receive.{u} {mailbox}
    ...
    | _ => False) :
      StrongRefinement.Aborting (· ∼[some (mailbox, inbox)] ·)
        ⟦GuardedPlusCal.Statement.receive (Typ := SurfaceTLAPlus.Typ) chan ref⟧⊥
        (⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ)
          ...
          ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ)
            ...
            (⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) ref
              ...
              ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) ref
                ...
                ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) { name := inbox, args := [] }
    ...
    · -- `await Len(inbox) > 0` cannot abort
      ...
      1,2:
        apply CoreTLAPlus.neval_gt_spec at eval_len
        obtain eval_len|eval_0 := eval_len
        · specialize eval_len (.int vs.length) _ vs.length rfl
          · apply CoreTLAPlus.eval_Len_intro (vs := vs)
            · exact CoreTLAPlus.eval_Len
            · erw [CoreTLAPlus.eval_var, dif_neg inbox_not_prim, AList.lookup_union_eq_some]
              refine Or.inr ⟨?_, ?_⟩
        ...
        · specialize eval_0 (.int 0) _ 0 rfl
          ...
          · contradiction
      1,2:
        apply CoreTLAPlus.eval_gt_spec at eval_len
    ...
    · -- `ref := Head(inbox)` may abort
      ...
      · -- inbox is in Mₜ (by eval_len) ↯
        ...
        apply CoreTLAPlus.neval_Head CoreTLAPlus.eval_Head at eval_inbox
        ...
        erw [CoreTLAPlus.eval_nat_iff, Nat.repr_toInt!] at eval_0
        cases eval_0
      ...
      · obtain ⟨arg, arg_in, e, e_in, _, _, _, eval_e, _|_, rfl⟩ := abort_head
        ...
        · rw [← eval_e]
          ...
          · specialize inbox_fresh_in_ref_args _ arg_in _ e_in
            rw [List.singleton_disjoint]
            exact CoreTLAPlus.not_mem_of_fresh inbox_fresh_in_ref_args
      · obtain ⟨_, _, _, v, vss, eval_head, eval_args, upd, ref_name_not_in_Tₛ, _|_, rfl⟩ := abort_head

        apply CoreTLAPlus.eval_Head_to_is_nonempty_seq CoreTLAPlus.eval_Head at eval_head
        ...
        obtain ⟨mailbox, rfl⟩|⟨mailbox, rfl⟩ := relatesTo_mailbox_shape sim
          ...
          <;> {
            have vs'_eq : vs' = v :: vs := by
              ...
              erw [CoreTLAPlus.eval_var, dif_neg inbox_not_prim] at eval_inbox eval_inbox'
              rw [AList.lookup_union_eq_some] at eval_inbox'

              have : AList.lookup inbox Tₛ ≠ some (CoreTLAPlus.Value.seq vs''') := by
              ...
              have : AList.lookup inbox (Tₛ ∪ Mₜ) = some (CoreTLAPlus.Value.seq vs') := by
            ...
            | some vs'' =>
              ...
              · rw [← List.forall₂_iff_forall₂_attach] at eval_args ⊢
                ...
                · rw [List.singleton_disjoint]
                  specialize inbox_fresh_in_ref_args _ arg_in _ e_in
                  apply CoreTLAPlus.not_mem_of_fresh inbox_fresh_in_ref_args
              · unfold args
    ...
    · -- `inbox := Tail(inbox)` cannot abort
      ...
      have inbox_in_Mₜ : inbox ∈ Mₜ := by
        erw [CoreTLAPlus.eval_var, dif_neg inbox_not_prim, AList.lookup_union_eq_some] at eval_inbox
      ...
      · obtain ⟨_, _, _, neval_inbox, _|_, rfl⟩ := abort_tail
        ...
        apply CoreTLAPlus.neval_Tail CoreTLAPlus.eval_Tail at neval_inbox
        ...
        · obtain rfl := List.eq_of_mem_singleton e'_in
          rw [← eval_inbox]
          apply CoreTLAPlus.eval_ext [ref.name]
          · intros x x_nin
          ...
          · rwa [List.singleton_disjoint, CoreTLAPlus.Expression.freeVars, List.mem_singleton]
        · nomatch neval_inbox
      · nomatch abort_tail
      · obtain ⟨_, _, _, v', vss, eval_tail, eval_args, nupd, lookup_inbox_Tₜ, _|_, rfl⟩ := abort_tail
        ...
        apply CoreTLAPlus.eval_Tail_is_seq CoreTLAPlus.eval_Tail at eval_tail
  ...
  private theorem AtomicBranch.correctness_precond_receive'.reorder_await.{u}
    ...
    (newInstrs_shape : ∀ (k : Nat) (_ : k < newInstrs.length),
        ...
        (Even k → ∃ r, r.name ≠ inbox ++ nameₛ ∧ r.name ∈ Γ \ {"self"} ∧ newInstrs[k] = ⟨r ,.opcall (.var "Head") [.var (inbox ++ nameₛ)]⟩) ∧
        (Odd k → inbox ++ nameₛ ∉ «Σ» \ {"self"} ∧ newInstrs[k] = ⟨⟨inbox ++ nameₛ, []⟩, .opcall (.var "Tail") [.var (inbox ++ nameₛ)]⟩)) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (0 : Nat).repr)) .«>» (.nat s!"{i}"))⟧* ∘ᵣ₂
        List.foldl (λ x (r, e) ↦ x ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e⟧*) {⟨x, e, y⟩ | x = y ∧ e = Trace.τ} newInstrs =
      List.foldl (λ x (r, e) ↦ x ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e⟧*) {⟨x, e, y⟩ | x = y ∧ e = Trace.τ} newInstrs ∘ᵣ₂
        ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (newInstrs.length / 2).repr)) .«>» (.nat s!"{i}"))⟧* := by
    conv_lhs =>
      enter [1, 1, 1, 1, 3, 1, 1]
    ...
    | cons_cons xs x y ys xs_len_even ys_len_even IH =>
      ...
      · conv_rhs =>
        ...
        · congr 1
          ...
          obtain ⟨ref, _, _, name_in, rfl⟩ := even_shape xs_len_even
          obtain ⟨-, -, -, odd_shape⟩ := newInstrs_shape₂
          obtain ⟨_, inbox_not_in, rfl⟩ := odd_shape (Even.add_one xs_len_even)
          ...
          have inbox_neq_Len : inbox ++ nameₛ ≠ "Len" := by
            ...
            apply Finset.not_mem_subset (t := CoreTLAPlus.prims.{u}.toFinmap.keys)
            · rw [CoreTLAPlus.prims_toFinmap_keys_eq, ← Finset.inter_eq_right]
              rfl
            · apply Finset.not_mem_subset «prims_in_Σ»
              assumption
          ...
          have ref_name_neq_Len : ref.name ≠ "Len" := by
            rw [← Finset.notMem_singleton]
            apply Finset.not_mem_subset (t := CoreTLAPlus.prims.{u}.toFinmap.keys)
            · rw [CoreTLAPlus.prims_toFinmap_keys_eq.{u}, ← Finset.inter_eq_right]
              rfl
            · apply Finset.not_mem_subset «prims_in_Σ»
              apply Disjoint.notMem_of_mem_right_finset «Σ_disj_Γ»
              assumption
          ...
          · injection h with decide_eq
            ...
            apply CoreTLAPlus.eval_Len_is_int CoreTLAPlus.eval_Len at len_inbox_eq
            obtain ⟨_, vs, eval_inbox_eq, _|_⟩ := len_inbox_eq

            dsimp [GuardedPlusCal.Ref.substOf]
            change CoreTLAPlus.eval M (CoreTLAPlus.Expression.replace (CoreTLAPlus.Expression.replace _ _ _) _ _) = _
            conv_lhs =>
              enter [2, 1]
              dsimp
              ...
              <;> {
                ...
                · apply CoreTLAPlus.eval_plus_intro
                  · erw [Int.ofNat_lt, List.length_pos_iff_exists_cons] at n₁_pos
                    ...
                    · apply CoreTLAPlus.eval_Tail_intro v' rfl CoreTLAPlus.eval_Tail
                      assumption
                    · rfl
                  · apply CoreTLAPlus.eval_nat_intro
                    erw [Nat.repr_toInt!]
          ...
          · dsimp [GuardedPlusCal.Ref.substOf] at eval
            change CoreTLAPlus.eval M (CoreTLAPlus.Expression.replace (CoreTLAPlus.Expression.replace _ _ _) _ _) = _ at eval
            conv_lhs at eval =>
              enter [2, 1]
              dsimp
              unfold CoreTLAPlus.Expression.replace
            ...
            split_ifs at eval with args_empty
              ...
              <;> {
                ...
                apply CoreTLAPlus.eval_Len_is_int CoreTLAPlus.eval_Len at len_tail_inbox_eq
                obtain ⟨_, vs, eval_tail_inbox_eq, _|_⟩ := len_tail_inbox_eq
                apply CoreTLAPlus.eval_Tail_is_seq CoreTLAPlus.eval_Tail at eval_tail_inbox_eq
                obtain ⟨_, v', _, eval_inbox_eq, _|_⟩ := eval_tail_inbox_eq

                refine ⟨_, _, ⟨vs.length + 1, xs.length / 2, ?_, ?_, rfl⟩, rfl, ?_⟩
                · apply CoreTLAPlus.eval_Len_intro
                  · apply CoreTLAPlus.eval_Len
                  · exact eval_inbox_eq
                  · rfl
                  · rfl
  ...
  private theorem AtomicBranch.correctness_precond_receive'.reorder_await'.{u}
    ...
    (newInstrs_shape : ∀ (k : Nat) (_ : k < newInstrs.length),
        ...
        (Even k → ∃ r, r.name ≠ inbox ++ nameₛ ∧ r.name ∈ Γ \ {"self"} ∧ newInstrs[k] = ⟨r, .opcall (.var "Head") [.var (inbox ++ nameₛ)]⟩) ∧
        (Odd k → inbox ++ nameₛ ∉ «Σ» \ {"self"} ∧ newInstrs[k] = ⟨⟨inbox ++ nameₛ, []⟩, .opcall (.var "Tail") [.var (inbox ++ nameₛ)]⟩)) :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (toString (0 : Nat)))) .«>» (.nat s!"{i}"))⟧⊥
        ∪ ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (toString (0 : Nat)))) .«>» (.nat s!"{i}"))⟧*
        ∘ᵣ₁ List.foldr (λ x y ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ y) ∅ newInstrs ⊆
      newInstrs.foldr (λ x sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem)
        ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (toString (newInstrs.length / 2)))) .«>» (.nat s!"{i}"))⟧⊥ := by
    ...
    | cons_cons xs y₁ y₂ ys even_xs_len even_ys_len IH =>
      ...
      conv at IH =>
        ...
        change ?await ⊆ List.foldr
          (λ x y ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ y)
          ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ)
              (CoreTLAPlus.Expression.infix (Typ := SurfaceTLAPlus.Typ)
        ...
        · rw [NetworkPlusCal.Statement.reorder_await y₂_name_not_prim,
          ...
          · rw [NetworkPlusCal.Statement.reorder_await y₁_name_not_prim,
            ...
            · apply NetworkPlusCal.Statement.abort_await_of_imp
              ...
              dsimp [GuardedPlusCal.Ref.substOf]
              change CoreTLAPlus.eval M (CoreTLAPlus.Expression.replace (CoreTLAPlus.Expression.replace _ _ _) _ _) = v
            ...
            · apply Relation.lcomp₁.subset_of_subset_left
              ...
              simp_rw [CoreTLAPlus.eval_gt_iff, CoreTLAPlus.eval_plus_iff, CoreTLAPlus.eval_nat_iff] at eval_Len_gt
    ...
    (inv : match (motive := _ → Prop) B with
      ...
      | .some B =>
        StrongRefinement
          ...
          (List.foldr (λ S sem ↦ ⟦S⟧⊥ ∪ ⟦S⟧* ∘ᵣ₁ sem) ∅ Ss)
          (List.foldr (λ S sem ↦ ⟦S⟧∞ ∪ ⟦S⟧* ∘ᵣ₁ sem) ∅ Ss)
          (⟦B⟧* ∘ᵣ₂ List.foldl (λ sem (x : _ × _) ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*) {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} newInstrs)
          (⟦B⟧⊥ ∪ ⟦B⟧* ∘ᵣ₁ newInstrs.foldr (λ ⟨r, e⟩ sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧* ∘ᵣ₁ sem) ∅)
          (⟦B⟧∞ ∪ ⟦B⟧* ∘ᵣ₁ newInstrs.foldr (λ ⟨r, e⟩ sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧∞ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧* ∘ᵣ₁ sem) ∅)) :
    ----
      i + 1 = (newInstrs ++
      ...
      StrongRefinement
        ...
        (List.foldr (λ S sem ↦ ⟦S⟧∞ ∪ ⟦S⟧* ∘ᵣ₁ sem) ∅ (Ss ++ [GuardedPlusCal.Statement.receive chan ref]))
        (B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₂
          ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ)
          ...
          List.foldl (λ sem x ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) x.1 x.2⟧*)
          ...
          (B.elim {x | x.1 = x.2.2 ∧ x.2.1 = Trace.τ} (⟦·⟧*) ∘ᵣ₂
            ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ)
              (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ)
                ...
                CoreTLAPlus.InfixOperator.«>»
                (CoreTLAPlus.Expression.nat (toString i)))⟧*) ∘ᵣ₁
            List.foldr (λ ⟨r, e⟩ sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧* ∘ᵣ₁ sem) ∅
              (newInstrs ++
                [(ref, CoreTLAPlus.Expression.opcall (CoreTLAPlus.Expression.var "Head") [CoreTLAPlus.Expression.var (inbox ++ nameₛ)]),
                 ({ name := inbox ++ nameₛ, args := [] }, CoreTLAPlus.Expression.opcall (CoreTLAPlus.Expression.var "Tail") [CoreTLAPlus.Expression.var (inbox ++ nameₛ)])]))
        (B.elim ∅ (⟦·⟧∞) ∪
  ...
  have reorder_await' :
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (0 : Nat).repr)) .«>» (.nat s!"{i}"))⟧* ∘ᵣ₂
        List.foldl (λ x (r, e) ↦ x ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e⟧*) {⟨x, e, y⟩ | x = y ∧ e = Trace.τ} newInstrs =
      List.foldl (λ x (r, e) ↦ x ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := CoreTLAPlus.Typ) r e⟧*) {⟨x, e, y⟩ | x = y ∧ e = Trace.τ} newInstrs ∘ᵣ₂
        ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (newInstrs.length / 2).repr)) .«>» (.nat s!"{i}"))⟧* :=
    AtomicBranch.correctness_precond_receive'.reorder_await.{0} «Σ_disj_Γ» «prims_in_Σ» i_eq newInstrs_len_even newInstrs_shape

  have await_eq {i : Nat} :
      ⟦NetworkPlusCal.Statement.await.{0} (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«>» (.nat s!"{i}"))⟧* =
      ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ) (.infix (.opcall (.var "Len") [.var (inbox ++ nameₛ)]) .«+» (.nat (0 : Nat).repr)) .«>» (.nat s!"{i}"))⟧* := by
    apply NetworkPlusCal.Statement.sem_await_congr'
    intro M
    simp_rw [CoreTLAPlus.eval_gt_iff, CoreTLAPlus.eval_plus_iff, CoreTLAPlus.eval_nat_iff]
  ...
  · intros k k_le
    ...
    · dsimp
      ...
      · apply Finset.mem_union_left
        apply Finset.mem_union_left
        apply «prims_in_Σ»
        simp [CoreTLAPlus.prims_keys_eq]
      · apply Finset.mem_union_left
        apply Finset.mem_union_right
        apply Finset.mem_union_right
        apply Finset.mem_singleton_self
    · intros arg arg_in idx idx_in
      apply CoreTLAPlus.wellscoped_mono_of_subset (h₄ _ arg_in _ idx_in)
  ...
  · have B_div_empty : B.elim (∅ : Set (_ × _)) (⟦·⟧∞) = ∅ := by
    ...
    have newInstrs_div_empty : (newInstrs ++
        ...
        (λ (x : _ × _) sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧∞ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) x.1 x.2⟧* ∘ᵣ₁ sem) = ∅ := by
      clear * -
    ...
    · simp_rw [sem_assoc, abort_assoc]
      ...
      · -- reduce `await Len(inbox) + n > n` to `await Len(inbox) > 0`
        ...
        have : -- TODO: extract as lemma?
            ?await = ⟦NetworkPlusCal.Statement.await (Typ := CoreTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := CoreTLAPlus.Typ)
          ...
          apply and_congr_right'
          apply and_congr_left'
          rw [CoreTLAPlus.eval_plus_gt_eq_gt_minus, CoreTLAPlus.eval_infix_congr]
          · rfl
          · unfold CoreTLAPlus.eval
    ...
    · conv =>
      ...
      have reorder_await_newInstrs' :
          ⟦NetworkPlusCal.Statement.await (Typ := SurfaceTLAPlus.Typ) (CoreTLAPlus.Expression.infix (Typ := SurfaceTLAPlus.Typ)
              (CoreTLAPlus.Expression.opcall (CoreTLAPlus.Expression.var "Len") [CoreTLAPlus.Expression.var (inbox ++ nameₛ)])
        ...
        · intros M
          ...
          rw [CoreTLAPlus.eval_nat_minus, Nat.sub_self]
          rfl
      ...
      · conv =>
        ...
        · rw [i_eq, Nat.sub_self,
          ...
          · apply GuardedPlusCal.Statement.strong_refinement.receive.{0}
            ...
            · assumption
          · intros M
            simp_rw [CoreTLAPlus.eval_gt_iff, CoreTLAPlus.eval_plus_iff]
            ...
            · erw [CoreTLAPlus.eval_nat_iff, Nat.repr_toInt!] at h₂
              cases h₂
  ...
  theorem AtomicBranch.correctness_precond_spec₁
    ...
    (inbox_fresh_in_Br : match (motive := _ → Prop) mailboxₛ with
      | .none => True
      | .some _ => inbox ++ nameₛ ∉ «Σ» \ {"self"} ∧ ∀ S ∈ precondₛ.toList, GuardedPlusCal.Statement.FreshIn CoreTLAPlus.Expression.FreshIn S (inbox ++ nameₛ))
    ...
    (precondₛ_wellscoped : GuardedPlusCal.AtomicBranch.WellScopedPrecond (flip CoreTLAPlus.Expression.WellScoped)
      ...
      | ⟨.some B, i, newInstrs, _⟩ => ⌜∃ Ξ',
        ...
        (if h : !Ss.isEmpty then
          ∃ (_ : Disjoint («Σ» \ {"self"}) Ξ') (_ : Disjoint (fifosₛ.keys.toFinset \ {"self"}) Ξ') (_ : Disjoint (Γ \ {"self"}) Ξ'),
            GuardedPlusCal.AtomicBranch.WellScopedPrecond (flip CoreTLAPlus.Expression.WellScoped) («Σ» \ {"self"}) (fifosₛ.keys.toFinset \ {"self"}) (Γ \ {"self"}) ‹_› ‹_› ‹_›
              (.some (GuardedPlusCal.Block.ofList Ss (propext List.isEmpty_eq_false_iff ▸ Bool.not_eq_true' _ ▸ h))) {"self"} Ξ'
        else Ξ' = {"self"}) ∧
        (∀ (k : Nat) (_ : k < newInstrs.length),
          (∀ arg ∈ newInstrs[k].1.args, ∀ idx ∈ arg, CoreTLAPlus.Expression.WellScoped idx ((«Σ» \ {"self"}) ∪ (Γ \ {"self"} ∪ {inbox ++ nameₛ}) ∪ Ξ')) ∧
        ...
        StrongRefinement (· ∼[Option.map (λ x ↦ (x, inbox ++ nameₛ)) mailboxₛ] ·)
          ...
          (⟦B⟧* ∘ᵣ₂ newInstrs.foldl (λ sem ⟨r, e⟩ ↦ sem ∘ᵣ₂ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧*) {(x, e, y) | x = y ∧ e = Trace.τ})
          (⟦B⟧⊥ ∪ ⟦B⟧* ∘ᵣ₁ newInstrs.foldr (λ ⟨r, e⟩ sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧⊥ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧* ∘ᵣ₁ sem) ∅)
          (⟦B⟧∞ ∪ ⟦B⟧* ∘ᵣ₁ newInstrs.foldr (λ ⟨r, e⟩ sem ↦ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧∞ ∪ ⟦NetworkPlusCal.Statement.assign (Typ := SurfaceTLAPlus.Typ) r e⟧* ∘ᵣ₁ sem) ∅)⌝) ?step
  ...
  theorem AtomicBranch.correctness_precond_spec
    ...
    (inbox_fresh_in_Br : match (motive := _ → Prop) mailboxₛ with
      | .none => True
      | .some _ => inbox ++ nameₛ ∉ «Σ» \ {"self"} ∧ ∀ S ∈ precondₛ.elim [] GuardedPlusCal.Block.toList, GuardedPlusCal.Statement.FreshIn CoreTLAPlus.Expression.FreshIn S (inbox ++ nameₛ))
    {«Σ_disj_Δ» : Disjoint («Σ» \ {"self"}) (fifosₛ.keys.toFinset \ {"self"})} {«Σ_disj_Γ» : Disjoint («Σ» \ {"self"}) (Γ \ {"self"})} {Δ_disj_Γ : Disjoint (fifosₛ.keys.toFinset \ {"self"}) (Γ \ {"self"})}
    {«Σ_disj_Ξ» : Disjoint («Σ» \ {"self"}) Ξ} {Δ_disj_Ξ : Disjoint (fifosₛ.keys.toFinset \ {"self"}) Ξ} {Γ_disj_Ξ : Disjoint (Γ \ {"self"}) Ξ}
    (precondₛ_wellscoped : GuardedPlusCal.AtomicBranch.WellScopedPrecond (flip CoreTLAPlus.Expression.WellScoped)
  ...
  theorem AtomicBranch.correctness_precond
    ...
    {rxs : List ((_ : String) × SurfaceTLAPlus.Typ × GuardedPlusCal.ChanRef (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))}
    (inbox_fresh_in_Br : match (motive := _ → Prop) mailboxₛ with
      | .none => True
      | .some _ => inbox ++ nameₛ ∉ «Σ» \ {"self"} ∧ ∀ S ∈ precondₛ.elim [] GuardedPlusCal.Block.toList, GuardedPlusCal.Statement.FreshIn CoreTLAPlus.Expression.FreshIn S (inbox ++ nameₛ))
    {«Σ_disj_Δ» : Disjoint («Σ» \ {"self"}) (fifosₛ.keys.toFinset \ {"self"})} {«Σ_disj_Γ» : Disjoint («Σ» \ {"self"}) (Γ \ {"self"})} {Δ_disj_Γ : Disjoint (fifosₛ.keys.toFinset \ {"self"}) (Γ \ {"self"})}
    ...
    --(wf : match (motive := _ → _ → Prop) mailboxₛ, precondₛ with | .none, _ | .some _, .none => True | .some _, .some precondₛ => inbox ++ nameₛ ∉ precondₛ.begin.flatMap (GuardedPlusCal.Statement.freeVars CoreTLAPlus.Expression.freeVars) ++ precondₛ.last.freeVars CoreTLAPlus.Expression.freeVars ∧ inbox ++ nameₛ ∉ CoreTLAPlus.prims)
  ...
  lemma GuardedPlusCal.Statement.abort_goto {label} :
      ⟦@GuardedPlusCal.Statement.goto SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) label⟧⊥ = ∅ := by
    ext ⟨⟨Mₛ, Tₛ, Fₛ, lₛ⟩, ε⟩
    iff_rintro ⟨_|_, _, _⟩ (_|_)

  lemma NetworkPlusCal.Statement.abort_goto {label} :
      ⟦@NetworkPlusCal.Statement.goto SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) label⟧⊥ = ∅ := by
    ext ⟨⟨Mₜ, Tₜ, Fₜ, lₜ⟩, ε⟩
    iff_rintro ⟨_|_, _, _⟩ (_|_)
  ...
  theorem AtomicBranch.correctness_action
    ...
    | case2 actionₜ S Ss h IH =>
      ...
      have wf' : match (motive := _ → Prop) mailbox with
          | none => True
          | some (fst, inbox) => GuardedPlusCal.Statement.FreshIn CoreTLAPlus.Expression.FreshIn S' inbox := by
        cases mailbox with
        | none => apply True.intro
      ...
      have wellscoped :
          ...
          | .skip => True
          | .print e => CoreTLAPlus.Expression.WellScoped e ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ)
          | .assert e => CoreTLAPlus.Expression.WellScoped e ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ)
          | .send chan e =>
            CoreTLAPlus.Expression.WellScoped e ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ) ∧ chan.name ∈ (fifosₛ.keys.toFinset \ {"self"}) ∧ (∀ idx ∈ chan.args, CoreTLAPlus.Expression.WellScoped idx ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ))
          | .multicast chan bs e =>
            chan ∈ (fifosₛ.keys.toFinset \ {"self"}) ∧ (∀ r ∈ bs, CoreTLAPlus.Expression.WellScoped r.2.2.2 ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ)) ∧ (∀ r ∈ bs, r.1 ∉ («Σ» \ {"self"}) ∧ r.1 ∉ (Γ \ {"self"}) ∧ r.1 ∉ Ξ) ∧
            CoreTLAPlus.Expression.WellScoped e ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ ∪ (bs.map Prod.fst).toFinset)
          | .assign ref e =>
            CoreTLAPlus.Expression.WellScoped e ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ) ∧ ref.name ∈ (Γ \ {"self"}) ∧ (∀ arg ∈ ref.args, ∀ idx ∈ arg, CoreTLAPlus.Expression.WellScoped idx ((«Σ» \ {"self"}) ∪ (Γ \ {"self"}) ∪ Ξ)) := by
  ...
  theorem AtomicBranch.correctness
    {nameₛ : String} {mailboxₛ : Option (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)}
    {Brₛ : GuardedPlusCal.AtomicBranch SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ)}
    {fifosₛ : Std.HashMap String (SurfaceTLAPlus.Typ × List (CoreTLAPlus.Expression SurfaceTLAPlus.Typ))}
    {localsₛ : Std.HashMap String (SurfaceTLAPlus.Typ × Bool × CoreTLAPlus.Expression SurfaceTLAPlus.Typ)}
    (Brₛ_wf : Brₛ.wellFormed inbox mailboxₛ nameₛ fifosₛ)
    {precondₜ : Option (GuardedPlusCal.Block (NetworkPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) true) false)}
    {newInstrs : List (NetworkPlusCal.Statement SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) false false)}
    {«Σ» Ξ : Scope}
    ...
    (Br_action_wellscoped : GuardedPlusCal.AtomicBranch.WellScoped (flip CoreTLAPlus.Expression.WellScoped) { precondition := none, action := Brₛ.action }
      («Σ» \ {"self"}) (fifosₛ.keys.toFinset \ {"self"}) (localsₛ.keys.toFinset \ {"self"}) Ξ)
    (self_in_Ξ : "self" ∈ Ξ)
    (inbox_fresh_in_Brₛ : match (motive := _ → Prop) mailboxₛ with
      | .none => True
      | .some _ => inbox ++ nameₛ ∉ «Σ» ∧ GuardedPlusCal.AtomicBranch.FreshIn CoreTLAPlus.Expression.FreshIn Brₛ (inbox ++ nameₛ))
    («prims_in_Σ» : CoreTLAPlus.prims.{0}.keys.toFinset ⊆ «Σ») :
  ...
  private theorem Process.correctness
    ...
    («inbox_fresh_in_Pₛ_Σ» : match Pₛ.mailbox with
      | .none => True
      | .some _ => inbox ++ Pₛ.name ∉ «Σ» ∧ GuardedPlusCal.Process.FreshIn CoreTLAPlus.Expression.FreshIn Pₛ (inbox ++ Pₛ.name))
      ...
      · simp_rw [List.unzip₃_snd, List.unzip_snd, List.mem_map] at Tₜ_in
        ...
        case inv1 =>
          exact (⇓ (⟨_, Bsₛ, _⟩, r) =>
            ...
            ∀ Bₛ ∈ Bsₛ, ∀ Br ∈ Bₛ.branches,
              ...
              (match (motive := _ → Prop) mailboxₛ with | .none => True | .some _ => inbox ++ nameₛ ∉ «Σ» ∧ Br.FreshIn CoreTLAPlus.Expression.FreshIn (inbox ++ nameₛ)) ∧
              Br.WellScoped (flip CoreTLAPlus.Expression.WellScoped) («Σ» \ {"self"}) ((channelsₛ.keys.toFinset ∪ fifosₛ.keys.toFinset) \ {"self"}) (localsₛ.keys.toFinset \ {"self"}) {"self"}
        ...
        case vc3.step.post.success =>
          ...
          · dsimp at h ⊢
            ...
            · exact h'₁ _ Bₜ_in
            · --rename' x => Bₛ
              rename GuardedPlusCal.AtomicBlock SurfaceTLAPlus.Typ (CoreTLAPlus.Expression SurfaceTLAPlus.Typ) => Bₛ
        ...
        case step =>
          ...
          have fresh : match (motive := _ → Prop) mailboxₛ with
              | .none => True
              | .some _ => inbox ++ nameₛ ∉ «Σ» \ {"self"} ∧ ∀ S ∈ Br.precondition.elim [] GuardedPlusCal.Block.toList, S.FreshIn CoreTLAPlus.Expression.FreshIn (inbox ++ nameₛ) := by
            cases mailboxₛ with
            | none => apply True.intro
            | some _ =>
          ...
          obtain _|⟨⟨x, τ, c⟩, _⟩ := r.2.2 <;> {
            ...
            all:
              ...
              · rw [List.forall₂_singleton]
                ...
                refine AtomicBranch.correctness Br_wf ?_ Br_action_wellscoped Ξ'_supset inbox_fresh_in_Br «prims_in_Σ»
                split at h <;> simp [*]
          }
    · contradiction
