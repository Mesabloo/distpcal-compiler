...
import PlusCalCompiler.Passes.SourceCodeToSurface.PlusCal
...
namespace SurfaceTLAPlus.Lexer
  ...
  -- private structure Located' (α : Type _) where
...
namespace SurfaceTLAPlus.Parser
  ...
  abbrev TLAPlusParser := SimpleParser (Parser.Stream.OfList (Located (Token (Located SurfacePlusCal.Token)))) (Located (Token (Located SurfacePlusCal.Token)))
  ...
  -/
  private def located {α} (p : TLAPlusParser α) : TLAPlusParser (Located α) := do
    let s ← getStream
  ...
  section Tokens
    ...
    private def parseIdentifier : TLAPlusParser (Located String) := do
      let ⟨pos, .identifier str⟩ ← withBacktracking <| tokenFilter λ | ⟨_, .identifier _⟩ => true | _ => false
        | unreachable!
      return ⟨pos, str⟩

    private def parseNumber : TLAPlusParser (Located String) := do
      let ⟨pos, .number raw⟩ ← withBacktracking <| tokenFilter λ | ⟨_, .number _⟩ => true | _ => false
        | unreachable!
      return ⟨pos, raw⟩

    private def parseString : TLAPlusParser (Located String) := do
  ...
  namespace Annotations
    ...
    private def parseAnnotation : TypeParser CommentAnnotation := do
      ...
      let args ← eoption <| first [
        ...
        do
          ...
          let _ ← _root_.Parser.token ';'
          return [Sum.inl <| String.mk chars.toList],
      ]
    ...
    where
      ...
      arg : TypeParser (String ⊕ Int ⊕ Bool ⊕ 𝒱) := first [
        (.inl ∘ String.mk ∘ Array.toList) <$> (char '"' *> takeMany stringChar <* char '"'),
    ...
    private def tryParseAnnotations : TLAPlusParser (List (Located CommentAnnotation)) := do
      let s ← getStream
  ...
  section Expressions
    private def parseInfixOperator (ws : TLAPlusParser PUnit) : TLAPlusParser (Located InfixOperator) := debug "infix op" <| lexeme do
      match ← withBacktracking <| ws *> tokenFilter λ | ⟨_, .infix _⟩ | ⟨_, .prefix .«-»⟩ => true | _ => false with
        | ⟨pos, .infix op⟩ => return ⟨pos, op⟩
        | ⟨pos, .prefix .«-»⟩ => return ⟨pos, .«-»⟩
        | ⟨_, _⟩ => unreachable!

    private def parsePrefixOperator : TLAPlusParser (Located PrefixOperator) := do
      let ⟨pos, .prefix op⟩ ← withBacktracking <| tokenFilter λ | ⟨_, .prefix _⟩ => true | _ => false
        | unreachable!
      return ⟨pos, op⟩

    private def parsePostfixOperator : TLAPlusParser (Located PostfixOperator) := do
      let ⟨pos, .postfix op⟩ ← withBacktracking <| tokenFilter λ | ⟨_, .postfix _⟩ => true | _ => false
        | unreachable!
      return ⟨pos, op⟩

    inductive OperatorOrExpression : Type _
      | «prefix» (_ : Located PrefixOperator)
      | «postfix» (_ : Located PostfixOperator)
      | «infix» (_ : Located InfixOperator)
      | atom (_ : Located (Expression (Located CommentAnnotation)))
      | index (_ : Located (Bool × List (Located (Expression (Located CommentAnnotation)))))
    ...
    section ShuntingYardAlgorithm
      ...
      -/
      def shuntingYard (input : List OperatorOrExpression) : TLAPlusParser (Located (Expression (Located CommentAnnotation)))
        := do
          let instToStringLocated' {α} [ToString α] : ToString (Located α) := {
            toString := λ x ↦ toString x.data
          }

          let mut output : List (Located (Expression (Located CommentAnnotation))) := []
              ...
              | .index args => do
                ...
                while h : !opsStack.isEmpty do
                  ...
                  match o with
                    ...
                    | .infix op@⟨_, .«.»⟩ => output := pushOperatorOntoOutput (.infix op) output
                  ...
                  match o with
                    | .prefix opPre => do
                      ...
                      if HasPrecedence.blt opPre opIn then
                        ...
                        output := pushOperatorOntoOutput (.prefix opPre) output
                    | .infix opIn' =>
                      if opIn.data = opIn'.data then
                        match TLAPlus.InfixOperator.assoc opIn.data with
        ...
        where
          pushOperatorOntoOutput
            | .infix opIn, e₂@⟨p₂, _⟩ :: e₁@⟨p₁, _⟩ :: es => ⟨p₁.merge p₂, .infixCall e₁ opIn e₂⟩ :: es
            | .prefix opPre@⟨pOp, _⟩, e@⟨p, _⟩ :: es => ⟨p.merge pOp, .prefixCall opPre e⟩ :: es
            | .postfix opPost@⟨pOp, _⟩, e@⟨p, _⟩ :: es => ⟨pOp.merge p, .postfixCall e opPost⟩ :: es
            | .index ⟨p, funOrOp, args⟩, e@⟨p', _⟩ :: es => ⟨p.merge p', (if funOrOp then Expression.fnCall else .opCall) e args⟩ :: es
            | _, _ => unreachable!
    end ShuntingYardAlgorithm

    section
      variable (ws : TLAPlusParser PUnit) (expr : TLAPlusParser PUnit → TLAPlusParser (Located (Expression (Located CommentAnnotation))))

      private def parseIfThenElse : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located do
        let _ ← ws *> token .if
        let cond ← expr ws
        let _ ← ws *> token .then
        let t ← expr ws
      ...
      private def parseJList : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| aligned λ ws ↦ do
        let col ← getCol
      ...
      private def parseRecordLiteral : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> do
      ...
      private def parseQuantifierBound : TLAPlusParser (QuantifierBound (Located 𝒱) (Located CommentAnnotation) (Located (Expression (Located CommentAnnotation)))) := first [
      ...
      private def parseFunctionLiteral : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> brackets do
        let qs ← Array.toList <$> sepBy1 (ws *> comma) (parseQuantifierBound ws expr)
        let _ ← ws *> token .«|->»
        let expr ← expr ws
        return .fn qs expr

      private def parseSetLiteral : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> braces do
        let es ← sepBy (ws *> comma) (expr ws)
        return .set es.toList

      private def parseTupleLiteral : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> angles do
        let es ← sepBy (ws *> comma) (expr ws)
        return .tuple es.toList

      private def parseQuantifier : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> do
      ...
      private def parseExcept (expr' : TLAPlusParser PUnit → Bool → TLAPlusParser (Located (Expression (Located CommentAnnotation)))) : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> brackets do
        let e ← expr ws
      ...
      private def parseSetMap : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> braces do
        let e ← expr ws
        let _ ← ws *> token .colon
        let qs ← sepBy1 (ws *> comma) (parseQuantifierBound ws expr)
        return .map' e qs.toList

      private def parseFunctionSet : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> brackets do
        let e₁ ← expr ws
        let _ ← ws *> token .«->»
        let e₂ ← expr ws
        return .fnSet e₁ e₂

      private def parseCase : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := located <| ws *> do
        let _ ← token .case
    ...
    mutual
      private partial def parseAtom (ws : TLAPlusParser PUnit) (inUpdate : Bool := false) : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := first [
        (.nat <$> ·) <$> parseNumber,
        (.var <$> ·) <$> parseIdentifier,
        (.str <$> ·) <$> parseString,
      ...
      partial def parseExpression (ws : TLAPlusParser PUnit := pure ()) (inUpdate : Bool := false) : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := debug "expression" <| withErrorMessage "expected expression" do
  ...
  private def parseExtends : TLAPlusParser (List (Located String)) := do
    let _ ← lexeme <| token .extends
    let mods ← sepBy1 (lexeme comma) parseIdentifier
    return mods.toList

  private def parseAssume : TLAPlusParser (Located (Expression (Located CommentAnnotation))) := debug "assume" do
    let _ ← withBacktracking <| token .assume
    parseExpression

  private def parseConstants : TLAPlusParser (List (Located String × List (Located CommentAnnotation))) := debug "constant" do
  ...
  private def parseVariables : TLAPlusParser (List (Located String × List (Located CommentAnnotation))) := debug "variables" do
  ...
  private def parseOperator : TLAPlusParser (List (Located CommentAnnotation) × Located String × List (Located String × Nat) × Located (Expression (Located CommentAnnotation))) := debug "operator def" do
    let ann ← tryParseAnnotations
    let var ← parseIdentifier
  ...
  private def parseDeclaration : TLAPlusParser (Located (Option (Declaration (Located CommentAnnotation)))) := located <| first [
  ...
  private def parsePlusCalAlgorithm : TLAPlusParser (Located (SurfacePlusCal.Algorithm (Located CommentAnnotation))) := do
  ...
  /-- Parse a full module. -/
  def parseModule' : TLAPlusParser (Located (Module (Located (SurfacePlusCal.Algorithm (Located CommentAnnotation))) (Located CommentAnnotation))) := located do
    ...
    return {
      name
      «extends» := exts.getD []
      declarations₁ := decls₁.toList.filterMap λ ⟨pos, x⟩ ↦ Located.mk pos <$> x
      pcalAlgorithm := alg
      declarations₂ := decls₂.toList.filterMap λ ⟨pos, x⟩ ↦ Located.mk pos <$> x
    }

  def parseModule (tokens : Array (Located (Token (Located SurfacePlusCal.Token)))) :
    Unexpected (Token (Located SurfacePlusCal.Token)) ⊕ Located (Module (Located (SurfacePlusCal.Algorithm (Located CommentAnnotation))) (Located CommentAnnotation)) :=
