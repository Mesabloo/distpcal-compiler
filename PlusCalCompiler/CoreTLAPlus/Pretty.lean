...
namespace CoreTLAPlus
  ...
  partial def Expression.pretty (e : Expression Typ) (prec : Nat) : Std.Format := match_source e with
    ...
    | .seq es, pos => formatPos pos ++ " " ++ f!"Seq(<<{Std.Format.joinSep (es.map (·.pretty 0)) ", "}>>)"
    | except fn upds, pos =>
