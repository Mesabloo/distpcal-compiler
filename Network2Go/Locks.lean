...
/-!
  Lock inference — deciding which process-local variables share a lock, in what order the locks are
  acquired, and which of them can be dropped.
  ...
  **Why locks at all.** A process's threads run as goroutines over shared process-local state, and
  an atomic block must observe that state as one indivisible step. Go offers no atomicity, so locks
  recover it — and then the whole question is how *few* locks suffice, since one global lock per
  process would serialize threads that never touch the same variable.
  ...
  **The scheme.** `shared` is the set of process-local variables a
  ...
  - **Footprints are per branch**, not per atomic block. The branch is what executes atomically; the
    ...
    `processFootprints` for why the block-level reading serializes branches that should run
    concurrently.
  - **`self` is never locked**, being read-only. Nothing special is needed for that here: `self` is
    bound by `checkProcess` rather than declared in `localState.variables`, and only declared
    variables are considered, so it
  ...
  - **A receiving thread has a footprint of its own, over its `inbox`.** `Thread.rx` has no label,
    ...
    `tmp1`.
  - **Thread-confinement pruning is *not* implemented.** A lock touched from only one thread does
    guard nothing —
    ...
    local of the thread function that started the chain. Worth revisiting only together with that.
-/
...
subtracted from *later* statements only — a temporary is not part of the footprint, but a process
variable of the same name read *before* the `with` was still read. -/
...
/--
  ...
  **Per branch, not per block.** The unit that executes atomically is the branch; a block only
  chooses among its branches, and locks are acquired per branch for exactly that reason. Taking the
  block's footprint — the union over its branches — makes two branches that touch disjoint variables
  look like joint users of both, so those variables dominate each other, merge into one lock, and
  each branch then acquires a lock bundling the other's variables. That serializes precisely the
  pair that should stay concurrent. Branch footprints are the finer relation and merge strictly less;
...
/-- `x ⪰ y`: every block whose footprint contains `y` also contains `x`. -/
private def dominates (fps : List (List String)) (x y : String) : Bool :=
  fps.all λ fp ↦ !fp.contains y || fp.contains x

/-- Merging along strict domination. Lock identities are represented by the variable that originally
...
Both the iteration order and the choice among several dominators are fixed to declaration order. Any
choice yields a valid assignment, but a compiler that emitted a different lock grouping for the same
input on different runs would be untestable.
