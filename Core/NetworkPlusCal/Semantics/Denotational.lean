...
import Extra.Rel
import Core.TypedSetTheory.Semantics.Operational
...
namespace NetworkPlusCal
  export TypedSetTheory (Value Memory)
  ...
  def Statement.reducing : {b b' : Bool} → Statement TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ) b b' → Set (LocalState false × List Behavior × LocalState b')
  ...
  where
    ...
    test (e : TypedSetTheory.Expression TypedSetTheory.Typ) (v : Value) : Set (LocalState false × List Behavior × LocalState false) :=
  ...
  def Statement.aborting : {b b' : Bool} → Statement TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ) b b' → Set (LocalState false × List Behavior)
    | true, false, .let name _ «=|∈» e =>
  ...
  def Statement.diverging : {b b' : Bool} → Statement TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ) b b' → Set (LocalState false × List Behavior)
    -- There is no divergence possible when reducing statements
    | _, _, _ => ∅

  @[reducible] instance instReduceStatement {b b' : Bool} : Reduce (Statement TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ) b b') (Set (LocalState false × List Behavior × LocalState b')) := ⟨Statement.reducing⟩
  @[reducible] instance instAbortStatement {b b' : Bool} : Abort (Statement TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ) b b') (Set (LocalState false × List Behavior)) := ⟨Statement.aborting⟩
  @[reducible] instance instDivergeStatement {b b' : Bool} : Diverge (Statement TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ) b b') (Set (LocalState false × List Behavior)) := ⟨Statement.diverging⟩
  ...
  instance instReduceAtomicBranch.{u} : Reduce (AtomicBranch.{u} TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ)) (Set.{u} (LocalState.{u} false × List Behavior.{u} × LocalState.{u} true)) where
    reducing B := B.precondition.elim NetworkPlusCal.Statement.reducing.id.{u} (⟦·⟧*) ∘ᵣ₂ ⟦B.action⟧*
  instance instAbortAtomicBranch : Abort (AtomicBranch TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ)) (Set (LocalState false × List Behavior)) where
    abort B := match B.precondition with
      | .none => ⟦B.action⟧⊥
      | .some B' => ⟦B'⟧⊥ ∪ ⟦B'⟧* ∘ᵣ₁ ⟦B.action⟧⊥
  instance instDivergeAtomicBranch : Diverge (AtomicBranch TypedSetTheory.Typ (TypedSetTheory.Expression TypedSetTheory.Typ)) (Set (LocalState false × List Behavior)) where
