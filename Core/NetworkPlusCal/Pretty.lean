...
import Core.GuardedPlusCal.Pretty
...
namespace NetworkPlusCal
  ...
  def Thread.pretty : Thread Typ Expr → Std.Format
    | .code T => .bracket "{" (.line ++ .joinSep T .line ++ .line) "}"
    | .rx chan var τ inbox => f!"(* @rx from {chan} into {inbox} by {var} : {τ} *) " ++ "{}"
  ...
  def Process.pretty (P : Process Typ Expr) : Std.Format :=
    ...
    let vars := if P.locals.isEmpty then .nil else .group (.indent 4 <|
      "variables " ++ .nest 4 (.joinSep (P.locals.toList.map λ ⟨name, τ, «=|∈», e⟩ ↦ Std.Format.line ++ name ++ " : " ++ Std.format τ ++ (if «=|∈» then " = " else " ∈ ") ++ Std.format e) ", ")
        ++ ";") ++ .line
