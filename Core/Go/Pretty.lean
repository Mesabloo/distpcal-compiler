...
namespace GoCal
  open TypedSetTheory (Typ Expression)
  ...
  partial scoped instance instToFormatExpression : Std.ToFormat (Expression Typ) where
    format :=
      ...
      let rec go (e : Expression Typ) (prec : Nat) : Std.Format := match_source e with
        | .var name _, pos => formatPos pos ++ sanitize name
        | .str raw, pos => formatPos pos ++ f!"`{raw}`"
        | .nat raw, pos => formatPos pos ++ f!"{raw}"
        | .bool raw, pos => formatPos pos ++ f!"{raw}"
        | .set elems τ, pos => formatPos pos ++ f!"SetLiteral[{τ}](" ++ Std.Format.joinSuffix (elems.map (go · 0)) ", " ++ ")"
        | .record fields, pos =>
        ...
        | .seq es τ, pos => formatPos pos ++ f!"SeqLiteral[{τ}]({Std.Format.joinSuffix (es.map (go · 0)) ", "})"
  ...
  private instance : Std.ToFormat (LHS Typ Expr) where
