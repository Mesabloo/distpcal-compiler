...
-- TODO: move into `Extra/List.lean`
namespace List
  ...
  theorem traverse_eq_none {α β : Type _} (f : α → Option β) {xs} :
    ...
    | cons x xs IH =>
      simp_rw [List.traverse_cons', seq_eq_bind_map, Option.bind_eq_bind,
...
namespace TypedSetTheory
  ...
  termination_by e
  decreasing_by
    all_goals subst_vars; try decreasing_trivial
    · rename String × Typ × Expression Typ => f
      calc
  ...
  theorem eval_Head.{u} {τ} {M : Memory.{u}} : M ⊢ .var "Head" τ ⇒ .prim "Head" := by
    unfold eval
    rfl
  ...
  theorem eval_Tail.{u} {τ} {M : Memory.{u}} : M ⊢ .var "Tail" τ ⇒ .prim "Tail" := by
    unfold eval
    rfl
  ...
  theorem eval_Len.{u} {τ} {M : Memory.{u}} : M ⊢ .var "Len" τ ⇒ .prim "Len" := by
    unfold eval
    rfl

  theorem eval_var.{u} {τ} {M : Memory.{u}} {x} : eval M (.var x τ) = if h : x ∈ prims.{u} then primFromName x h else M.lookup x := by
    unfold eval
    rfl

  theorem eval_var_irrel_type.{u} {τ τ'} {M : Memory.{u}} {x} : eval M (.var x τ) = eval M (.var x τ') := by
    simp [eval_var]
