...
namespace TypedSetTheory
  ...
  def eval.{u} (M : Memory.{u}) : Expression Typ → Option Value.{u}
    | .var name _ => if h : name ∈ prims.{u} then primFromName name h else M.lookup name
    | .str raw => return .str raw
    | .nat raw => return .int (String.toInt! raw)
    | .bool raw => return .bool raw
    | .set elems _ => .set <$> elems.attach.traverse λ ⟨e, _⟩ ↦ eval M e
    ...
    | .access e x => do
      ...
      bs.lookup x
    | .seq es _ => .seq <$> es.attach.traverse λ ⟨e, _⟩ ↦ eval M e
    | .opcall fn args => do
      let args ← args.attach.traverse λ ⟨e, _⟩ ↦ eval M e
