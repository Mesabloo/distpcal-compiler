...
namespace TypedSetTheory
  ...
  inductive Expression.{u} (Typ : Type u) : Type u
    ...
    /-- A boolean literal. -/
    | bool (raw : Bool)
    /-- A set literal `{e₁, …, eₙ} : 𝒫(τ)`. -/
    | set (elems : List (Expression Typ)) (τ : Typ)
    /-- A record literal `[x₁ : τ₁ ↦ e₁, …, xₙ : τₙ ↦ eₙ]`, with types ascribed to the fields. -/
    ...
    /-- A literal sequence `Seq(⟨e₁, …, eₙ⟩) : Seq[τ]`. -/
  ...
  def Expression.freeVars.{u} {Typ : Type u} : Expression Typ → List String
    ...
    | .nat _ => []
    | .bool _ => []
    | .set elems _ => elems.attach.flatMap λ ⟨e, _⟩ => e.freeVars
    ...
    | .seq es _ => es.attach.flatMap λ ⟨e, _⟩ => e.freeVars
  ...
  def Expression.replace (e : Expression Typ) (v : String) (e' : Expression Typ) : Expression Typ := match_source e with
    | .var name τ, pos => if v = name then e' else .var name τ @@ pos
    | .str raw, pos => .str raw @@ pos
    | .nat raw, pos => .nat raw @@ pos
    | .bool raw, pos => .bool raw @@ pos
    | .set elems τ, pos => .set (elems.attach.map λ ⟨e, _⟩ => e.replace v e') τ @@ pos
    ...
    | .seq es τ, pos => .seq (es.attach.map λ ⟨e, _⟩ ↦ e.replace v e') τ @@ pos
  ...
  partial instance {Typ : Type _} : ToString (Expression Typ) where
    toString :=
      let rec go : Expression Typ → String
        ...
        | .nat raw => s!"{raw}"
        | .bool raw => s!"{raw}"
        | .set elems _ => "{" ++ String.intercalate ", " (elems.map go) ++ "}"
        ...
        | .access e x => s!"({go e}).{x}"
        | .seq es _ => s!"Seq(<<{String.intercalate ", " (es.map go)}>>)"
