...
namespace GuardedPlusCal
  def Ref.pretty.{u} {Typ Expr : Type u} [Std.ToFormat Expr] (r : Ref Typ Expr) : Std.Format :=
    let args := r.args.map λ es ↦ .sbracket <| .joinSep es ", "
    r.name ++ .join args
  instance {Typ Expr : Type _} [Std.ToFormat Expr] : Std.ToFormat (Ref Typ Expr) := ⟨Ref.pretty⟩

  def ChanRef.pretty.{u} {Typ Expr : Type u} [Std.ToFormat Expr] (c : ChanRef Typ Expr) : Std.Format :=
    let args := if c.args.isEmpty then .nil else .sbracket <| .joinSep c.args ", "
    c.name ++ args
  instance {Typ Expr : Type _} [Std.ToFormat Expr] : Std.ToFormat (ChanRef Typ Expr) := ⟨ChanRef.pretty⟩
