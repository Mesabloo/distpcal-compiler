...
namespace GuardedPlusCal
  ...
  def Memory.updateRef (M : Memory) (r : Ref Typ Value) (v : Value) : Option Memory := do
  ...
  where
    doUpdate (v' : Value) : Value → List (List Value) → Option Value
      | _, [] => return v'
      | fn@(.fn maps), v :: vs => do
        let tup := if let [v] := v then v else TypedSetTheory.Value.tuple v
  ...
  def Statement.reducing : {b b' : Bool} → Statement Typ (Expression Typ) b b' → Set (LocalState false × List Behavior × LocalState b')
  ...
  where
    ...
    test (e : Expression Typ) (v : Value) : Set (LocalState false × List Behavior × LocalState false) :=
  ...
  def Statement.aborting : {b b' : Bool} → Statement Typ (Expression Typ) b b' → Set (LocalState false × List Behavior)
    | true, false, .let name _ «=|∈» e =>
  ...
  def Statement.diverging : {b b' : Bool} → Statement Typ (Expression Typ) b b' → Set (LocalState false × List Behavior)
    -- There is no divergence possible when reducing statements
    | _, _, _ => ∅

  instance instReduceStatement {b b' : Bool} : Reduce (Statement Typ (Expression Typ) b b') (Set (LocalState false × List Behavior × LocalState b')) := ⟨Statement.reducing⟩
  instance instAbortStatement {b b' : Bool} : Abort (Statement Typ (Expression Typ) b b') (Set (LocalState false × List Behavior)) := ⟨Statement.aborting⟩
  instance instDivergeStatement {b b' : Bool} : Diverge (Statement Typ (Expression Typ) b b') (Set (LocalState false × List Behavior)) := ⟨Statement.diverging⟩
  ...
  instance instReduceAtomicBranch : Reduce (AtomicBranch Typ (Expression Typ)) (Set (LocalState false × List Behavior × LocalState true)) where
    reducing B := B.precondition.elim {⟨x, e, y⟩ | x = y ∧ e = 1} (⟦·⟧*) ∘ᵣ₂ ⟦B.action⟧*
  instance instAbortAtomicBranch : Abort (AtomicBranch Typ (Expression Typ)) (Set (LocalState false × List Behavior)) where
    abort B := match B.precondition with
      | .none => ⟦B.action⟧⊥
      | .some B' => ⟦B'⟧⊥ ∪ ⟦B'⟧* ∘ᵣ₁ ⟦B.action⟧⊥
  instance instDivergeAtomicBranch : Diverge (AtomicBranch Typ (Expression Typ)) (Set (LocalState false × List Behavior)) where
