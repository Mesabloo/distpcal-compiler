...
namespace TypedPlusCal
  open SurfacePlusCal (MulticastFilter Declarations)
  ...
  mutual
    inductive Statement (α β : Type) : Bool → Type
      ...
      | assign (_ : List (Ref α β × β)) : Statement α β false
      ...
      | receive (c : Ref α β) (r : Ref α β) : Statement α β false
      | send (c : Ref α β) (e : β) : Statement α β false
  ...
  mutual
    partial def Statement.bimap {b} {α β γ δ} (f : α → β) (g : γ → δ) (S : Statement α γ b) : Statement β δ b := match_source S with
      ...
      | .assign asss, pos => .assign (bimap (bimap f g) g <$> asss) @@ pos
      | .if e B₁ B₂, pos => .if (g e) (Block.bimap f g B₁) (Block.bimap f g B₂) @@ pos
      | .await e, pos => .await (g e) @@ pos
      | .with vars B, pos => .with ((bimap f (Bifunctor.snd g) <$> ·) <$> vars) (Block.bimap f g B) @@ pos
      | .assert e, pos => .assert (g e) @@ pos
      | .either Bs, pos => .either (Branches.bimap f g Bs) @@ pos
      | .while e B, pos => .while (g e) (Block.bimap f g B) @@ pos
      | .receive c r, pos => .receive (bimap f g c) (bimap f g r) @@ pos
      | .send c e, pos => .send (bimap f g c) (g e) @@ pos
  ...
  mutual
    partial def Statement.bitraverse {F : Type _ → Type _} [Applicative F] {α β γ δ b} (f : α → F β) (g : γ → F δ) (S : Statement α γ b) : F (Statement β δ b) := match_source S with
      ...
      | .assign asss, pos =>
        (.assign · @@ pos) <$> traverse (bitraverse (bitraverse f g) g) asss
      | .if e B₁ B₂, pos =>
        (.if · · · @@ pos)
          <$> g e
      ...
      | .with vars B, pos =>
        (.with · · @@ pos)
          <$> traverse (traverse (bitraverse f (traverse g))) vars
          <*> Block.bitraverse f g B
      ...
      | .receive c r, pos => (.receive · · @@ pos) <$> bitraverse f g c <*> bitraverse f g r
      | .send c e, pos => (.send · · @@ pos) <$> bitraverse f g c <*> g e
